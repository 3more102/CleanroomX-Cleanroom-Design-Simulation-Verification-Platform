...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and atomic temporary-file replacement on save. Persisted spatial metadata is delegated to the GUI-independent `src/cleanroomx/spatial_schema.py` contract, which version-gates the nested layout, preserves extension fields, rejects ambiguous persisted identities, and deterministically assigns missing identities.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, canonicalize supported spatial metadata, and use an atomic replacement. The nested spatial layout has its own version-1 contract: unsupported future versions and malformed persisted collection shapes fail closed, duplicate explicit room/device ids are rejected as ambiguous, missing ids are allocated deterministically, and unknown extension fields survive normalization.
