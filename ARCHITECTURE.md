...
2. **Application service registry and plugin boundary** — `src/cleanroomx/application.py` defines the core workflow catalog, validates parser/runner/reporter contracts, and discovers optional trusted extensions through the versioned `cleanroomx.analysis_plugins` entry-point group. `src/cleanroomx/plugin_api.py` is the narrow public plugin contract. Plugin workflow keys are provider-namespaced, invalid plugins are rejected atomically without mutating the core catalog, and plugin identity is retained in run provenance.
...
A project file is loaded through `load_project_document()` and migrated only when it matches a supported legacy shape. Core analysis kinds map to fixed application-registry entries; installed plugin kinds are resolved only through the validated API-v1 registry. Ordinary workflows pass through declared parser and runner functions; consistency and dossier use explicit custom application adapters. Results are normalized to strict JSON and rendered through the backend reporter when available. Projects that reference an unavailable plugin fail closed with an actionable format error instead of reinterpreting the workflow.
