...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, spatial sub-document validation, and atomic temporary-file replacement on save. `src/cleanroomx/spatial_schema.py` is the GUI-independent persisted-layout contract; malformed versions, duplicate spatial IDs, invalid dimensions/device types, and orphan room references fail closed instead of being silently repaired during file load/save.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document (including the versioned `spatial_layout` metadata sub-document when present), and use an atomic replacement. Spatial editor normalization is deliberately separate from persistence validation: transient input may be normalized for interaction, but persisted corruption is rejected rather than silently rewritten. Deterministic normalization uses stable suffix/fallback IDs so repeated handling of the same transient layout produces the same identity graph.
