...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, optimistic on-disk revision guards, durable same-directory replacement, and post-write byte verification.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement. The desktop binds an opened project to its stable SHA-256 content revision and refuses a guarded save when the destination changed. The atomic writer fsyncs the temporary file before replacement and, on POSIX, fsyncs the containing directory after replacement so the rename itself is durably committed. Explicit project saves then re-read a stable file revision and require its byte count and SHA-256 to match the exact validated serialization before the save is reported successful.
