...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Once a fresh background result is accepted, the desktop appends a bounded run-audit record to project metadata containing the exact input snapshot, the execution provenance, outcome status, and SHA-256 identities for result, diagnostics, report, and plot content. The retained ledger is hash chained and validated before read or append; pruning retains an anchor digest so the remaining chain is continuous. A corrupt ledger is not silently rewritten. Project saves remain schema version 1 and use the existing validated atomic replacement path.
