...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, explicit migration provenance for supported legacy shapes, duplicate-id and active-analysis checks, stable file-revision capture, and atomic temporary-file replacement on save.
...
A project file is loaded through the project persistence layer and migrated only when it matches a supported legacy shape. Migration-aware loading returns the validated current-schema model together with immutable source-format/version and migration-step provenance while legacy public loader APIs keep their existing return contracts. The desktop treats a migrated project as an unsaved copy and requires the first save to use a different path, preserving the original legacy bytes. The selected analysis kind maps to a fixed application-registry entry. Ordinary workflows pass through declared parser and runner functions; consistency and dossier use explicit custom application adapters. Results are normalized to strict JSON and rendered through the backend reporter when available.
