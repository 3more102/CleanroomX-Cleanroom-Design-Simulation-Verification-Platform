...
4. **Project edit history** — `src/cleanroomx/project_history.py` provides bounded, deep-copied transactional snapshots for non-spatial desktop edits. `src/cleanroomx/gui.py` validates the complete post-edit project before committing a history entry, rolls back failed mutations, invalidates cached engineering results on restore, and keeps spatial geometry on its independent history boundary.
