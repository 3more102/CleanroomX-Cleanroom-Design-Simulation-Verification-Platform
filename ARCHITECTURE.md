...
3. **Shared persistence primitive** — `src/cleanroomx/persistence.py` owns crash-safe UTF-8 replacement for user files: same-directory temporary writes, file flush/fsync, atomic replace, existing permission preservation, temporary-file cleanup on exceptions/interrupts, and parent-directory fsync where supported. Guarded saves can supply a final pre-replace revision check without weakening atomicity.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, external-revision conflict protection, and delegates writes to the shared persistence primitive while preserving the existing public `atomic_write_text()` API.
...
8. **CLI entry points** — `pyproject.toml` exposes CleanroomX commands for verification, HVAC, recovery, uncertainty, qualification, networks, fan studies, dossier/consistency, and the desktop GUI. Every CLI `--output` path uses the shared atomic persistence primitive rather than direct truncating writes.
