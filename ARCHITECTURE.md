...
Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. Recovery artifacts are atomically written in a user-specific recovery directory, retain source-project fingerprint evidence, and are bounded by project identity. The manager keeps ownership of a submitted Future until its completion callback has finalized the active/pending coordinator state; a Future becoming done is not treated as permission to start a replacement write. Completed worker writes are accepted only after their project epoch is revalidated under the autosave lifecycle lock; only accepted generations may rotate retained history. Explicit save, discard, and project-transition invalidation therefore make any older in-flight completion stale: its just-written artifact is removed without evicting previously accepted recovery generations. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
