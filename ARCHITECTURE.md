...
3. **Project persistence and document lifecycle** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, collision-checked full-UUID allocation for newly created analyses, fail-closed analysis lookup/removal, and atomic temporary-file replacement on save. The desktop routes analysis creation/removal through this domain lifecycle instead of mutating the project list directly.
