...
9. **Runtime implementation provenance** — `src/cleanroomx/runtime_provenance.py` fingerprints the installed CleanroomX Python source tree with location-independent, newline-normalized SHA-256 identity plus Python/platform runtime identity. `run_analysis()` captures this evidence before validation and after result/report/plot construction; a changed source tree fails closed and the computed run is not published.
...
Every completed application run also records its configured validation/execution/reporting entry points, a before/after aggregate SHA-256 of all packaged `cleanroomx/*.py` sources, source-file count, and Python/platform runtime identity. The source aggregate excludes absolute installation paths and normalizes line endings so equivalent wheel/checkouts remain comparable across locations. If that implementation identity changes during execution, the run is discarded rather than emitted with ambiguous software provenance.
