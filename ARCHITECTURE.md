...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, analysis/spatial referential-integrity checks, and atomic temporary-file replacement on save.
4. **Spatial data model** — `src/cleanroomx/spatial_model.py` owns the versioned spatial-layout contract, deterministic normalization, stable-id/reference validation, analysis-derived seed geometry, and advisory geometry checks without importing Tk.
...
7. **Desktop UI** — `src/cleanroomx/gui.py` and `src/cleanroomx/spatial.py` provide project lifecycle, spatial interaction/rendering, JSON editing, validation, non-blocking execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, and active-run mutation guards.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document including any versioned spatial metadata, and use an atomic replacement. Spatial workspace refresh is read-only: persisted layouts are normalized into a working copy and verification-derived layouts remain transient until an explicit spatial edit persists them.
