...
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, and recovery scanning. New schema-v2 artifacts carry deterministic SHA-256 integrity evidence over the semantic payload; schema-v1 artifacts remain readable but are explicitly treated as legacy/unverified. It never writes the explicit project path.
