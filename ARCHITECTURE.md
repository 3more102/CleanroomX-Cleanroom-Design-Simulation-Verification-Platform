...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, guarded saves, and a shared durable atomic-write primitive. Writes use exact UTF-8 bytes, fsync the temporary file before replacement, preserve existing POSIX permission bits, fsync the parent directory where supported, and read back SHA-256/size evidence before a write is reported successful.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use the shared verified atomic replacement path. The verified revision returned to the GUI is the exact revision read back after replacement, so subsequent optimistic save protection remains bound to the bytes CleanroomX actually committed.
