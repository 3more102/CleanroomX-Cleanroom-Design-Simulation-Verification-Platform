...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, spatial-metadata integrity validation, and atomic temporary-file replacement on save.
4. **Spatial integrity** — `src/cleanroomx/spatial_integrity.py` owns the persisted spatial-layout contract independently from Tk: supported layout version, stable room/device identity, device-to-room referential integrity, finite/positive geometry, and persisted viewport bounds. `src/cleanroomx/spatial.py` remains responsible for interactive normalization, synchronized 2D/3D presentation, advisory geometric checks, and edit commands.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement. When `project.metadata.spatial_layout` is present, load and save validation fail closed on unsupported spatial versions, duplicate object ids, dangling device room references, non-finite coordinates, non-positive room dimensions, and persisted view values outside supported bounds. Interactive normalization remains deterministic and does not define the persistence contract.
