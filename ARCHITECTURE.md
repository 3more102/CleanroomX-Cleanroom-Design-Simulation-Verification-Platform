...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, guarded stable-revision writes, and a shared verified atomic-write primitive. The primitive flushes/fsyncs its same-directory temporary file, atomically replaces the destination, synchronizes the parent-directory rename where supported, and verifies the authoritative bytes by SHA-256 before reporting success.
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, and recovery scanning. Recovery schema v2 adds a canonical SHA-256 integrity record and still reads legacy v1 artifacts. It never writes the explicit project path.
...
Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. New recovery artifacts use `cleanroomx.autosave` schema v2 and carry a canonical SHA-256 over the complete envelope except the integrity block; v1 remains readable. Recovery artifacts are written through the same verified atomic primitive in a user-specific recovery directory, retain source-project fingerprint evidence, and are bounded by project identity. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
