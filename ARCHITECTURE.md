...
4. **Edit history** — `src/cleanroomx/edit_history.py` centralizes bounded, deep-copied undo/redo stack mechanics. `src/cleanroomx/spatial_history.py` and `src/cleanroomx/project_history.py` provide domain-specific state/no-op contracts so spatial and non-spatial project edits remain separate ownership domains. `src/cleanroomx/gui.py` validates the complete post-edit project before committing a project entry, rolls back failed mutations, and invalidates cached engineering results on restore.
...
Non-spatial project mutations in the desktop shell use a separate bounded edit history. Analysis input commits, add/rename/remove, imported inputs, project field edits, and explicit spatial-to-analysis synchronization pass through a validate-before-commit transaction boundary. Undo/redo restores a validated project snapshot and clears cached run evidence because the restored input revision may differ. Spatial layout metadata is intentionally excluded from this history and remains owned by the spatial workspace history, preventing an analysis undo from silently rewinding unrelated geometry. Save As to a different base directory clears project history because path-valued analysis inputs are rebased to the new project context.
