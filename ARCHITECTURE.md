...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and the shared verified atomic writer. The writer stages exact UTF-8 bytes in the destination directory, file-syncs and SHA-256 verifies them before replacement, preserves existing regular-file modes, and synchronizes the containing directory after replacement where supported.
