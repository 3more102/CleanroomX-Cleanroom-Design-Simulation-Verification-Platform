...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and verified same-directory atomic replacement. The shared writer emits exact UTF-8 bytes, flushes/fsyncs and re-verifies the staged file before replacement, fsyncs the parent directory where supported, then re-verifies the committed destination. Recovery artifacts plus desktop and CLI text/JSON/report exports use the same primitive.
...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Project saves serialize schema version 1 and validate the resulting document before persistence. The shared writer flushes and fsyncs a same-directory staging file, re-reads a stable staging revision and requires exact size/SHA-256 before the authoritative path can be replaced, then atomically replaces the destination, fsyncs the containing directory where supported, and re-verifies the committed destination. Staging/commit mismatches and unexpected directory-sync failures are explicit persistence errors rather than successful saves.
