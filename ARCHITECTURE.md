...
A completed `AnalysisRun` is also an immutable application snapshot: result JSON, diagnostics/provenance, and plot-model dict/list graphs are recursively copied and frozen when the run object is constructed. This prevents backend-retained references, GUI code, or extensions from mutating cached engineering evidence after acceptance. `AnalysisRun.to_dict()` deliberately thaws a detached ordinary-container copy for JSON/export callers, so serialization never exposes the cached graph itself.
