...
3. **Durable file persistence** — `src/cleanroomx/persistence.py` owns same-directory exclusive temporary writes, file-data sync, atomic replacement, exact post-replace byte/hash verification, POSIX directory sync, and existing-mode preservation. Project saves, recovery artifacts, GUI exports, and CLI file outputs share this path.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and guarded durable replacement on save while re-exporting the historical `atomic_write_text` API.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use the shared durable replacement path. Success is reported only after the exact committed bytes are re-read and verified; POSIX builds then sync the parent directory so the rename is persisted across a sudden host failure.
