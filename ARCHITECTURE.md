...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Background worker completion has an additional process-local context invariant: the originating project object, analysis object/active selection, and normalized base directory must still be current before either an error or a successful result can be published. Project replacement and base-rebasing workflows are blocked while a worker is active. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement.
