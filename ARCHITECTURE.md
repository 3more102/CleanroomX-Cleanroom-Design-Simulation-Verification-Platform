...
CleanroomX v0.99.0 is a Python 3.11+ engineering screening, simulation, verification, uncertainty/provenance, HVAC, fan/network, qualification, reporting, and desktop application platform. The desktop UI is an operator shell over the same validated backend modules used by the command-line tools.
...
2. **Application service registry** — `src/cleanroomx/application.py` declares the GUI-exposed workflows and validates key uniqueness, direct parser/runner contracts, custom adapters, and callable bindings.
...
4. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, background execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, and active-run/abandon worker guards.
