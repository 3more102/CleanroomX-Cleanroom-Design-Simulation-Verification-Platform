...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, exact-byte SHA-256 revision binding on load, guarded atomic replacement, and post-write content verification.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project loads parse one stable byte snapshot and derive the bound SHA-256 revision from those same bytes, avoiding a separate-read revision race. Guarded saves serialize and validate schema version 1, re-check the expected on-disk revision immediately before atomic replacement, then verify the resulting file revision against the exact serialized bytes before the save is accepted as successful. Recovery source fingerprints reuse the same project revision-capture primitive.
