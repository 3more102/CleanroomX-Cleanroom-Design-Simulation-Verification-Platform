...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, spatial-metadata integrity gating through `src/cleanroomx/spatial_schema.py`, and atomic temporary-file replacement on save.
...
Persisted `project.metadata.spatial_layout` data has its own version-1 integrity contract. Load/save validation rejects duplicate or blank room/device IDs, dangling device-to-room references, unsupported device types or spatial versions, non-finite coordinates, and non-positive room/grid dimensions before the workspace can normalize them. The Tk workspace may still use deterministic lenient normalization for transient/ad-hoc state, but persisted corruption is never silently repaired. Geometry advisories such as room overlap remain non-fatal editor warnings.
