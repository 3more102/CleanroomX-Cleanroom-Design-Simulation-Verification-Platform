...
Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. Recovery artifacts are atomically written in a user-specific recovery directory, retain source-project fingerprint evidence, and are bounded by both project identity and autosave session identity. Automatic history rotation can delete only validated artifacts owned by the current session; artifacts from another CleanroomX process, older session, or an unreadable recovery envelope are preserved for explicit recovery inspection. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
