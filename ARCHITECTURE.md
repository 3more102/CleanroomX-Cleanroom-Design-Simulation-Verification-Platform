...
6. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, non-blocking execution, per-analysis current-result ownership, bounded immutable session run history, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, and active-run mutation guards. `src/cleanroomx/run_history.py` owns history isolation and bounded retention independently of Tk widgets.
...
Result ownership remains tied to the analysis id, and each cached current result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and current result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Every accepted run is also copied into a bounded session history. The history is explicitly historical: it never becomes the current result automatically, exposes lightweight immutable summaries for list refresh, and deep-copies full retained evidence only on explicit retrieval/export. Project/recovery replacement and Save-As path-context changes reset the history. It is not persisted into the schema-v1 project document. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement.
