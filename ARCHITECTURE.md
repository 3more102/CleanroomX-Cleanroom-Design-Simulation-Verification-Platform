...
Every application run is also bound to the exact CleanroomX Python source revision executing it. `run_analysis()` deterministically fingerprints the package's `.py` source tree by relative path and exact bytes before execution and again before publishing the result. Exact hashes are cached only while a deterministic file identity/size/mtime/ctime manifest is unchanged. Diagnostics record the source-tree hash, source-file count, declared parser/runner/reporter or custom-adapter binding, Python/runtime platform metadata, and numerical runtime characteristics. If the source tree changes during a run, `RuntimeCodeChangedError` discards the result rather than publishing evidence assembled from a mixed software revision. This is intentionally independent of the package version because unreleased or editable builds can share one semantic version while containing different code.
