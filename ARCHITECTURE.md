...
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, canonical SHA-256 artifact integrity, post-write verification before history rotation, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, and recovery scanning. It never writes the explicit project path. Recovery schema v2 carries embedded integrity evidence; schema v1 remains readable as legacy/unverified for backward compatibility.
...
Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. Recovery schema v2 hashes the canonical JSON payload (excluding the integrity block) with SHA-256, atomically writes the artifact in the user-specific recovery directory, then reopens and verifies the persisted file before older generations may be rotated. If verification of a just-written artifact fails, the new artifact is removed best-effort and the previous good history is left untouched. Structurally valid but content-corrupted v2 artifacts discovered during later scans fail closed and are preserved as scan issues rather than offered for restore. Existing schema-v1 artifacts remain readable and are explicitly labeled legacy/unverified. The digest is corruption evidence, not a digital signature or proof of source authenticity. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
