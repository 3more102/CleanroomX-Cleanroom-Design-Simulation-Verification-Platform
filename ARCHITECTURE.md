...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Opening a project captures the exact stable file bytes and a SHA-256-backed file revision. Normal Save requires the current on-disk content to still match that revision immediately before atomic replacement and verifies the written bytes afterward. External replacement or deletion therefore leaves the in-memory project dirty instead of silently overwriting another writer's work; Save Project As remains the explicit path for preserving the local version separately.
