...
3. **Shared persistence** — `src/cleanroomx/persistence.py` owns dependency-light atomic text/byte replacement for project saves, recovery artifacts, desktop exports, and CLI reports. It writes exact UTF-8 bytes through a same-directory temporary file, flushes/fsyncs before replacement, preserves existing POSIX permission bits, and synchronizes the parent directory where supported. Genuine post-replace directory-sync failures are explicit committed-but-durability-unconfirmed errors.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and guarded save semantics on top of the shared persistence primitive. The historical `cleanroomx.project.atomic_write_text` import remains available as a compatibility re-export.
