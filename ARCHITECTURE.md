...
6. **Desktop task isolation** — `src/cleanroomx/analysis_task.py` executes GUI analyses in spawn-based child processes, returns strict-JSON outcomes through private temporary state, supports non-blocking terminate/kill cancellation, and converts abrupt worker exits into explicit task failures without changing the synchronous backend API.
7. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, cancellable non-blocking execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, and active-run mutation guards.
...
Desktop analysis execution is also outside the project model. The GUI copies validated input into a dedicated child process, so cancellation or a backend process failure cannot mutate the authoritative in-memory project. Only a successfully decoded `AnalysisRun` is attached to the owning analysis id. The CLI and backend APIs continue to call synchronous `run_analysis()` directly.
