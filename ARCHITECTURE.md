...
6. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, non-blocking execution, per-analysis result ownership, bounded in-session run-history inspection/export, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, and active-run mutation guards. `src/cleanroomx/run_history.py` stores historical runs as immutable canonical strict-JSON bundles with SHA-256 integrity, per-analysis retention, and a global memory budget.
...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Accepted runs are also copied into a separate bounded in-session history as canonical strict-JSON run bundles. Historical entries retain their input SHA-256 and bundle SHA-256, are never promoted to the current-result cache, and can be inspected/exported explicitly even after inputs change. Project replacement or path-context invalidation clears the session history; deleting one analysis clears only its history. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement.
