...
4. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, non-blocking execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, and active-run mutation guards, abandoned-worker exclusivity, and result-cache invalidation when Save As changes relative-path context.
