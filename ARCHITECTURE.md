...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, detached mutable-container ownership at model/snapshot boundaries, and atomic temporary-file replacement on save.
...
Project documents remain intentionally mutable inside the application, but constructor inputs and serialization snapshots are ownership boundaries. Nested analysis-input and project-metadata containers are detached when they enter the model, the analyses list container is owned by the project, and `to_dict()` returns detached nested data. Editing caller-owned parse/build dictionaries or a returned serialization snapshot therefore cannot mutate the authoritative project behind dirty tracking, autosave, result invalidation, or edit-history workflows.
