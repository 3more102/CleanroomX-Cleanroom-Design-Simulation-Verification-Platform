...
3. **Project persistence and spatial integrity** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and atomic temporary-file replacement on save. `src/cleanroomx/spatial_schema.py` owns the GUI-independent spatial sub-schema, deterministic normalization, stable-ID validation, and device-to-room referential-integrity checks used at the same project boundary.
