...
3. **Strict JSON and project persistence** — `src/cleanroomx/strict_json.py` owns deterministic JSON parsing/snapshot rules shared by the project layer and desktop editor, including duplicate-object-key and non-finite-value rejection. `src/cleanroomx/project.py` applies that boundary to the `cleanroomx.project` schema, supported legacy migration, duplicate-id and active-analysis checks, and atomic temporary-file replacement on save. The boundary also rejects values that Python's JSON encoder would otherwise silently coerce (for example non-string object keys and tuples), detects cycles and invalid UTF-8 surrogate text with JSON-path diagnostics, and prevents serialized snapshots from aliasing mutable live project state.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project ingress and serialization create detached strict-JSON snapshots, so caller-owned dictionaries cannot mutate live project state and serialization output cannot mutate the project by alias. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement.
