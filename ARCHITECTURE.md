...
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, and recovery scanning. The coordinator retains ownership of each background `Future` until its completion callback has finalized shared state, so a done-but-not-finalized write cannot be mistaken for idle work or overwrite tracking for a newer snapshot. It never writes the explicit project path.
