...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, optimistic external-write protection, canonical SHA-256 document integrity, durable atomic replacement, and post-save read-back verification.
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, canonical SHA-256 envelope integrity, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, and recovery scanning. It never writes the explicit project path.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1 with a canonical SHA-256 integrity record over the persisted envelope excluding that record itself. Existing schema-v1 files without integrity metadata remain readable. A save is not accepted until atomic replacement completes, the directory entry is fsynced where supported, and the resulting file is parsed back through the strict loader as a stable content revision. GUI saves retain the optimistic external-revision guard before replacement.

Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. New recovery artifacts carry their own canonical SHA-256 envelope digest covering project plus UI/editor draft state, are atomically written in a user-specific recovery directory, retain source-project fingerprint evidence, and are bounded by project identity. Pre-integrity schema-v1 recovery artifacts remain readable. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
