...
A project file is loaded through strict UTF-8/JSON parsing and migrated only when it matches a supported legacy shape. Desktop opens use `load_project_document_with_revision()`, which accepts a project only when the exact parsed byte stream hashes to the same stable revision observed around the read. The selected analysis kind maps to a fixed application-registry entry. Ordinary workflows pass through declared parser and runner functions; consistency and dossier use explicit custom application adapters. Results are normalized to strict JSON and rendered through the backend reporter when available.

Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, use a same-directory atomic replacement, verify the committed bytes, and flush the directory entry where supported. A post-replace durability-flush failure is surfaced distinctly: the desktop retains dirty/recovery state while adopting only the verified revision token needed for a safe retry.
