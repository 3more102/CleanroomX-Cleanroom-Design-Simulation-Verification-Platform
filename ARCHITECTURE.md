...
4. **Saved-version persistence** — `src/cleanroomx/saved_revisions.py` preserves the exact previous valid project bytes before explicit overwrite, verifies byte size/SHA-256/project validity, bounds history per source path, and restores only to an in-memory project copy.
...
Before an existing project path is explicitly overwritten, saved-version persistence takes a stable byte snapshot of that destination, validates it as a supported CleanroomX project, and atomically stores a separate checksummed historical artifact. Failure to preserve that pre-image aborts the overwrite. Historical restore revalidates the artifact and opens it as an unsaved copy with the original path retained only for relative-reference context, so rollback never silently rewrites the live project file.
