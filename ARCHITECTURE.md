...
Before an application run enters a backend, CleanroomX deep-copies the submitted JSON input and binds that isolated snapshot to its canonical SHA-256. Ordinary workflows parse that snapshot exactly once inside the run transaction and pass the resulting parsed model directly to the runner. The snapshot is re-hashed after parsing, after backend execution, and after plot construction; any in-place mutation raises `AnalysisInputMutationError` and discards the run. This prevents parser/runner aliasing or accidental mutation from changing the input revision described by execution provenance or from writing back into the project/editor model.
