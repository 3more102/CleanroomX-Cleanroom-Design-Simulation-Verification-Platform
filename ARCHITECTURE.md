...
6. **Desktop editing/UI** — `src/cleanroomx/project_history.py` provides bounded transactional history for analysis-document edits, while `src/cleanroomx/gui.py` integrates that history with project lifecycle, JSON editing, validation, non-blocking execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, and active-run mutation guards. Spatial geometry retains its separate `SpatialEditHistory` ownership boundary.
...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. Analysis-document undo/redo restores validated snapshots through the ordinary project parser and invalidates session run caches before exposing restored state. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement.
