...
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, recovery scanning, and bounded quarantine of unreadable artifacts. New schema-v2 artifacts carry deterministic SHA-256 integrity evidence over the semantic payload; schema-v1 artifacts remain readable but are explicitly treated as legacy/unverified. Quarantine preserves suspect bytes, records SHA-256/reason/timestamp audit metadata, and removes them from active recovery scans without rewriting the explicit project path.
5. **Recovery UI** — `src/cleanroomx/recovery_ui.py` provides startup recovery discovery, evidence inspection, explicit discard, invalid-artifact quarantine, and restore selection without owning project-save semantics.
