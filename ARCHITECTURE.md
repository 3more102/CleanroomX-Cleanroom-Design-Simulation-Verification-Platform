...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, enforce the opened on-disk content revision, preserve the previous valid explicit state before a changed overwrite, use an atomic replacement, then reload and verify the committed content before accepting the save.

Saved revisions are sidecar persistence rather than project-schema content. They retain exact prior project bytes plus SHA-256/size/timestamp/version/source-path evidence in a bounded history beside the source project. Restore validates the revision envelope and embedded project, refuses to overwrite its source, and writes only to a separately selected destination. A post-write verification rollback is conditional on proving the destination still contains CleanroomX's attempted write, preserving external-change protection across failure recovery.
