...
The same external-file provenance is now part of the centralized cached-result freshness invariant. Cache restore, worker acceptance, editor commit, and result/report export re-hash referenced files and compare SHA-256 plus byte size with the post-run revision. Changed, missing, or unstable dependencies invalidate the cached run; a metadata-only timestamp change remains valid when file content is identical.
