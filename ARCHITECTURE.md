...
6. **Desktop UI** — `src/cleanroomx/gui.py` provides project lifecycle, JSON editing, validation, non-blocking execution, per-analysis result ownership, diagnostics, reporting, export, plotting, dirty-state tracking, unsaved-change protection, recovery-autosave status, protected recovery restoration, active-run mutation guards, and run-history inspection.
7. **Analysis run audit history** — `src/cleanroomx/run_history.py` owns the bounded, integrity-checked execution ledger stored under the existing project metadata block. Accepted desktop runs retain exact input snapshots, execution provenance, outcome identity, and SHA-256 content digests without duplicating full result payloads.
...
Result ownership remains tied to the analysis id, and each cached result is bound to the canonical SHA-256 of the exact submitted analysis input already recorded in execution provenance. Cache restore, worker completion, and result/report export fail closed when the current analysis kind or input hash no longer matches that provenance, so missed mutation notifications cannot silently expose stale engineering evidence. The analysis worker prepares the potentially expensive result/diagnostic/report/plot SHA-256 identities before returning to Tk. Once that fresh background result is accepted, the UI appends a bounded run-audit record to project metadata containing the exact input snapshot, execution provenance, outcome status, and those prepared content identities. The retained ledger is hash chained and validated before read or append; pruning retains an anchor digest so the remaining chain is continuous. A corrupt ledger is not silently rewritten. Project saves remain schema version 1 and use the existing validated atomic replacement path.
...
## Run-history integrity boundary

The persisted run-history digest chain is designed to detect accidental project-data mutation or corruption and to preserve deterministic execution identity across application sessions. It is not a cryptographic authenticity signature: a party that can rewrite the project file can also recompute the unkeyed hashes. Full result/report content remains available through the existing run-bundle/report exports; the project ledger stores stable identities rather than duplicating potentially large result payloads.
