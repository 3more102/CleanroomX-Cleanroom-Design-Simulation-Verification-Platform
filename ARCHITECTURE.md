...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, same-directory temporary-file replacement, file flush/fsync, directory-entry fsync where supported, and post-replace SHA-256/size verification. The same primitive protects recovery artifacts plus desktop and CLI text/JSON/report exports.
