...
Current unreleased persistence hardening centralizes same-directory atomic UTF-8 text replacement in `src/cleanroomx/persistence.py`. Project saves, recovery artifacts, GUI exports through the compatibility re-export, and every CLI `--output` path now share the same flush/fsync/replace primitive instead of maintaining separate direct-write behavior.
...
3. **Text persistence primitive** — `src/cleanroomx/persistence.py` owns same-directory temporary-file creation, flush/fsync, pre-replace validation hooks, atomic replacement, and temporary-file cleanup on failure.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, external-revision save guards, and re-exports `atomic_write_text` for backward compatibility.
...
8. **CLI entry points** — `pyproject.toml` exposes CleanroomX commands for verification, HVAC, recovery, uncertainty, qualification, networks, fan studies, dossier/consistency, and the desktop GUI. File outputs use the shared atomic persistence primitive; stdout behavior is unchanged.
...
A project file is loaded through `load_project_document()` and migrated only when it matches a supported legacy shape. The selected analysis kind maps to a fixed application-registry entry. Ordinary workflows pass through declared parser and runner functions; consistency and dossier use explicit custom application adapters. Results are normalized to strict JSON and rendered through the backend reporter when available. CLI `--output` writes then use the shared persistence primitive, so a failed replacement does not first truncate an existing complete report or result file.

Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use the shared atomic replacement primitive; guarded saves inject the external-revision check immediately before replacement.
