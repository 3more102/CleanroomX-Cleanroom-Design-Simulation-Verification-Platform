...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, stable SHA-256/size source fingerprints, guarded stale-revision detection, and atomic temporary-file replacement on save.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project loads retain the exact stable SHA-256/byte-size fingerprint of the opened file. In-place saves compare that source revision immediately before atomic replacement and fail closed if the file was changed or removed externally; Save As to the currently opened path is routed through the same guard. Successful writes advance the session baseline to the exact serialized bytes CleanroomX wrote.
