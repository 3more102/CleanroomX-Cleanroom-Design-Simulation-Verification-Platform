...
1. **Engineering backends** — calculation, solver, uncertainty, qualification, HVAC, fan/network, dossier, consistency, and report modules under `src/cleanroomx/`. Duct-path, fixed-demand branch-flow, room thermal/air-balance, and supply-fan backends separate full-precision calculation state from rounded result presentation. HVAC composition consumes the full-precision layer for governing-airflow selection, filter counts, balance decisions, aggregate loads/flows, fan sizing, and fan-curve duty checks, so display rounding never becomes a downstream engineering input.
