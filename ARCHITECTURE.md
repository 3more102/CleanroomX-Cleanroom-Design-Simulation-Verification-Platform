...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, exact-byte SHA-256 revision tracking, and atomic temporary-file replacement with optional stale-revision rejection on save.
...
A project file is loaded from a stable byte snapshot and migrated only when it matches a supported legacy shape. The desktop retains the SHA-256 revision of the exact bytes it parsed so an ordinary Save can verify that the source file has not changed on disk. The selected analysis kind maps to a fixed application-registry entry. Ordinary workflows pass through declared parser and runner functions; consistency and dossier use explicit custom application adapters. Results are normalized to strict JSON and rendered through the backend reporter when available.

Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use an atomic replacement. For an already-open project, the replacement is conditional on the current file still matching the revision captured at open or the previous successful save; a mismatch preserves both the external file and the in-memory local edits and is reported as a save conflict.
