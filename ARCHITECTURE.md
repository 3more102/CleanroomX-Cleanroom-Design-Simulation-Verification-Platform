...
Spatial metadata uses deterministic canonical identity repair: valid room/device ids remain unchanged, missing or duplicate ids receive collision-safe deterministic suffixes, and declared ids are reserved before repair so malformed earlier entries cannot steal a later valid identity. Device-to-room references remain explicit; unresolved references are preserved for validation rather than guessed or silently rebound. This keeps repeated normalization, undo snapshots, recovery snapshots, and save/reload behavior reproducible without changing the project schema.
