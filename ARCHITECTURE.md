...
3. **Strict JSON boundary** — `src/cleanroomx/jsonio.py` centralizes standards-compliant JSON parsing for standalone engineering files and strict JSON serialization for CLI results. File ingestion rejects non-finite numeric constants before model construction, and CLI JSON output refuses to emit non-standard `NaN`/`Infinity` tokens.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and atomic temporary-file replacement on save.
