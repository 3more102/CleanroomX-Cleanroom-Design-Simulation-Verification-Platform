...
- **Strict JSON authority:** `cleanroomx.strict_json` owns canonical JSON parsing/snapshot rules. Project persistence and public file-backed engineering loaders reject duplicate object keys and non-finite values before model construction instead of relying on workflow-specific downstream checks.
