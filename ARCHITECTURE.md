...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, optimistic external-revision guards, and verified atomic replacement. Atomic text writes use canonical UTF-8 bytes, fsync and re-read the staged file before replacement, fsync the containing directory on POSIX after replacement, and re-read the committed destination before reporting success.
