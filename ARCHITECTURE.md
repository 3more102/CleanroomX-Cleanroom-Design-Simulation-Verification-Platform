...
3. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, optimistic external-revision guarding, and durable atomic replacement: same-directory temp write, file fsync, guarded rename, POSIX parent-directory fsync, and committed-content read-back verification.
4. **Recovery persistence** — `src/cleanroomx/autosave.py` owns separate versioned recovery artifacts, source-file fingerprints, bounded rotation, asynchronous write/coalescing, validated restore/discard operations, recovery scanning, and optional schema-v1 SHA-256 envelope integrity evidence for corruption detection. It never writes the explicit project path.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, preserve the existing optimistic external-revision checks, and use the shared durable atomic-write contract. A save/export/autosave is not reported as successful until the renamed target can be read back with the exact committed text; POSIX hosts also fsync the containing directory so the rename itself is included in the durability boundary.

Recovery autosave is deliberately outside the project schema. The UI captures a model snapshot plus raw draft state on the Tk thread and submits it to a single background writer. Recovery artifacts are atomically written in a user-specific recovery directory, retain source-project fingerprint evidence, and are bounded by project identity. New schema-v1 artifacts also carry a SHA-256 checksum over the canonical recovery envelope (excluding the integrity field); the loader rejects checksum mismatches while remaining backward-compatible with older schema-v1 artifacts that have no integrity block. Explicit saves remain authoritative; recovery artifacts are never substituted for or written over the project file.
