...
Completed file-backed runs are also treated as revision-bound evidence while they remain cached in the desktop. Before a cached consistency/dossier run is restored or exported, the application re-hashes each referenced file with the same stable-read primitive and compares its current content identity with the post-run fingerprint. Changed, missing, or unstable dependencies invalidate the cache and force a rerun; metadata-only timestamp changes with identical content remain valid.
