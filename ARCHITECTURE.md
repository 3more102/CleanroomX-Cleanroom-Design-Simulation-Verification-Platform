...
3. **Persistence primitive** — `src/cleanroomx/persistence.py` owns crash-hardened text mutation: exact UTF-8 staging, staged-file fsync, permission-mode preservation for existing regular files, atomic same-directory replacement, supported parent-directory fsync, and durable unlink.
4. **Project persistence** — `src/cleanroomx/project.py` implements the `cleanroomx.project` schema, strict JSON validation, supported legacy migration, duplicate-id and active-analysis checks, and guarded saves through the shared persistence primitive.
...
Result ownership remains tied to the analysis id so stale results are not silently reassigned after edits, deletion, or analysis switching. Project saves serialize schema version 1, validate the resulting document, and use the shared persistence transaction. The writer stages exact UTF-8 bytes in the destination directory, fsyncs the staged file, preserves the existing regular-file mode, performs the final project revision check, atomically replaces the destination, and then fsyncs directory metadata where the platform exposes that operation.
