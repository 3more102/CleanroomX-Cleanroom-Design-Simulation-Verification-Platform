...
7. **Runtime diagnostics** — `src/cleanroomx/runtime_diagnostics.py` provides an app-owned, thread-safe, bounded event journal and strict-JSON support bundles. It records lifecycle/failure metadata without copying full engineering inputs or results, strips directory context from referenced-file provenance, and writes diagnostic bundles through the existing atomic persistence primitive. No event is transmitted externally.
...
Runtime observability is instance-owned rather than global. The desktop records a bounded sequence of application, save/export, validation, and analysis-worker lifecycle events. **Export Diagnostic Bundle JSON...** serializes those events with environment metadata, project structure, runtime state, and sanitized execution provenance. The bundle intentionally omits full analysis inputs/results and retains only file basenames plus hash/size/stability evidence for external dependencies; export is local, explicit, and atomic.
