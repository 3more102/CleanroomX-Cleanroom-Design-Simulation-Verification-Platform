...
- Uses explicit fixed quadratic edge laws `deltaP = R * Q * abs(Q)` with balanced user-supplied node injections.
- Supports signed reverse flow and reference-node-independent edge-flow solutions.
...
- Keeps v0.21 automatic Darcy-friction calculations separate: v0.23 loop edges use explicit fixed resistance and do not iterate geometry/friction with solved flow.
