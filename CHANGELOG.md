...
## v0.84 independent internal network-state fingerprint replay — 2026-09-24
...
- Canonical state covers solved node pressures/balances, edge flows/resistances and pressure-law residuals, pressure-power balance, and variable-friction closure evidence while excluding prose-only scope text.
...
- Detects internal network-state provenance corruption even when fan pressure, loop terminal pressure, total system pressure, residuals, bracket geometry, and v0.83 full-bracket scalar component replay all remain unchanged.
- Propagates network-state replay coverage and violation corners through standalone reports, nonlinear uncertainty summaries, engineering dossiers, and solved/iteration-limit regressions.
- Preserves root selection, bisection decisions, fan interpolation, nonlinear solver tolerances, no-extrapolation behavior, and all existing engineering acceptance semantics.
