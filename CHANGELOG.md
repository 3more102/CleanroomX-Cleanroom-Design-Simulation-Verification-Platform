...
- Adds regressions for duplicate-key rejection, custom-adapter contract enforcement, metadata consistency, headless GUI exposure, and abandoned-worker execution exclusivity.
- Hardens GUI **Abandon** so abandoned results/errors are suppressed while the application remains busy until the backend worker actually exits, preventing overlapping backend analyses.
