...
- Hardens the shared project/autosave/export atomic-write primitive: temporary UTF-8 bytes are flushed and file-`fsync()`ed before replacement, the containing directory entry is `fsync()`ed where supported, and the committed destination is re-read from a stable path identity and checked against the intended byte size and SHA-256.
- Raises an explicit `AtomicWriteVerificationError` when the replaced destination does not match the submitted bytes, preventing the desktop from treating an unverified persistence operation as a successful save.
...
- Preserves project schema version 1, public save call shapes, solver equations, numerical tolerances, acceptance semantics, and the existing optimistic external-write guard.
- Adds regression coverage for directory-sync invocation, post-replace corruption detection, ordinary atomic replacement cleanup, and the existing guarded-save conflict paths; focused desktop CI now includes `test_project_write_guard.py`.
