...
## v0.19 automatic Darcy friction screening — 2026-09-23
...
- Rejects automatic laminar friction for noncircular ducts instead of applying an unvalidated circular-duct approximation; users can still supply an explicit friction factor.
- Preserves the existing explicit `friction_factor` workflow unchanged and rejects ambiguous manual-plus-automatic inputs.
- Resolves branch-tree friction after terminal-demand airflows are propagated, so each branch uses its solved airflow.
- Resolves reference-flow fan/duct friction at the declared reference section flow, then holds that resolved factor constant while deriving the bounded fixed-ratio quadratic system curve.
- Leaves the passive parallel-flow and fan-driven parallel-network solvers on explicit fixed resistance; v0.19 does not add nonlinear variable-friction network balancing.
- Added focused regression coverage, documentation, an automatic-friction HVAC example, and v0.19.0 metadata.
