...
- Iteration-limit bisection decision-trace replay for v0.75, retaining each completed L/H midpoint decision when the search exhausts its iteration budget, replaying the final endpoint replacement into the remaining signed-residual bracket, and preserving non-converged status without a fabricated T decision or operating point.
