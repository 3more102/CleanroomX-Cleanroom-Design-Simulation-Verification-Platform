...
The v0.25 workflow derives each edge's fixed quadratic resistance from explicit duct geometry, air density, local-loss coefficient, and either a supplied Darcy factor or the existing automatic-friction inputs evaluated at a declared reference airflow. It then reuses the v0.23.1 loop solver. The derived resistance is frozen during mesh balancing; Reynolds number and friction factor are not iterated as solved flow changes. See docs/LOOPED_DUCT_NETWORK.md.
