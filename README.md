...
- Connected looped-network airflow solving from explicit balanced node injections and fixed quadratic edge resistances, with signed reverse-flow and residual reporting.
...
Next milestones are integrating looped-network studies into engineering dossiers, deriving loop-edge resistances from explicit duct geometry, coupling looped networks to bounded fan studies, closed-loop control studies, and later a desktop/web UI plus CFD adapters.
