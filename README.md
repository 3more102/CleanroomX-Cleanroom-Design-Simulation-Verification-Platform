...
Fan operating-point plots reuse backend-computed system-pressure evidence and render labeled fan/system series. Interactive desktop analyses run in an isolated spawned child process; **Cancel** terminates that computation, waits for confirmed process exit before unlocking another run, and never publishes a cancellation-racing result. An unexpected analysis-process exit is reported without taking down the Tk/project process. The installed wheel includes the self-contained demo and supports `cleanroomx-gui --demo`.
