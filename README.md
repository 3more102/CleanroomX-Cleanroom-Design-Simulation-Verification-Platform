...
- Signed-zero-stable network-state canonical projection for v0.92, recursively normalizing floating `-0.0` to `0.0` before projection comparison and SHA-256 hashing so zero-sign representation cannot create false replay mismatches while nonzero state changes remain detectable.
