...
v0.27 variable-friction loop solving re-evaluates automatic Darcy friction from each edge's absolute solved airflow, relaxes geometry-derived resistance updates, and repeats the validated fixed-resistance loop solve until resistance closure is reached. Direct resistance inputs and user-supplied friction factors remain fixed; zero-flow branches are not assigned invented Reynolds values.
...
The v0.27 workflow iterates only geometry-derived edges configured with automatic roughness/viscosity friction inputs. Explicit resistance and user-supplied-friction edges remain fixed. See docs/VARIABLE_FRICTION_LOOP.md.
