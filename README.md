...
v0.100 records a canonical SHA-256 identity for each submitted analysis input. File-backed `consistency` and `dossier` workflows also record dependency SHA-256 and byte-size evidence before and after execution, surfaced in Diagnostics and preserved by **Export Run Bundle JSON**. Dossiers containing only absolute references can run before the GUI project is saved; relative references still require a saved project/base directory. Import and **Save Project As** preserve the referents of relative consistency/dossier files across directory-context changes, while cached results are cleared when the base directory changes. Project saves and desktop exports use same-directory atomic UTF-8 replacement, fan workflows plot backend system-curve evidence alongside supplied fan curves when available, and the GUI validates registry/catalog parity before creating the Tk root and keeps an abandoned analysis exclusive until its backend worker exits. The installed distribution also ships a self-contained demonstration available through `cleanroomx-gui --demo`, with wheel-level CI smoke coverage.
