...
- Iteration-limit decision-trace replay provenance for v0.75, retaining the complete unresolved L/H path, auditing its expected no-T termination pattern, and replaying the final endpoint-replacement decision into the retained remaining signed-residual bracket with exact uncertainty-corner violation provenance.
