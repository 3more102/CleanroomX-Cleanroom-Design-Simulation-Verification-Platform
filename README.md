...
- Geometry-derived looped duct-network solving that freezes each edge into a quadratic resistance from explicit duct geometry, density, local losses, and a reference-flow Darcy factor before mesh balancing.
...
## Solve a looped duct network from geometry
...
The v0.24 workflow derives each edge's fixed quadratic resistance from explicit duct geometry, air density, local-loss coefficient, and either a supplied Darcy factor or the existing automatic-friction inputs evaluated at a declared reference airflow. It then reuses the v0.23 loop solver. The derived resistance is frozen during mesh balancing; Reynolds number and friction factor are not iterated as solved flow changes. See docs/LOOPED_DUCT_NETWORK.md.
...
Next milestones are bounded fan/loop-network coupling, closed-loop control studies, and later a desktop/web UI plus CFD adapters.
