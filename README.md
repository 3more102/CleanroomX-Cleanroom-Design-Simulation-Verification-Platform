...
- Connected looped-network airflow solving from explicit balanced node injections and fixed quadratic edge resistances, with signed reverse flow plus continuity and pressure-law residuals.
...
The v0.20 solver accepts a connected graph with explicit balanced node injections/withdrawals and fixed positive edge resistances using `ΔP = R·Q·|Q|`. It solves relative node pressures and signed edge flows with a damped Newton method, reports continuity and pressure-law residuals, and permits flow reversal relative to the declared edge direction. It does not infer resistance from geometry or couple the mesh to fan/control models. See docs/LOOPED_NETWORK_SOLVER.md.
...
Next milestones are integrating fan-speed and looped-network studies into engineering dossiers, deriving loop-edge resistances from explicit duct geometry, coupling looped networks to bounded fan studies, and later a desktop/web UI plus CFD adapters.
