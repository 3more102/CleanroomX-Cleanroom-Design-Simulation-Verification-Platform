...
- Evaluated-corner fan power evidence for nonlinear uncertainty, with complete-study fluid-air-power ranges, explicitly enabled shaft/electrical/SFP ranges, and tie-aware source-corner provenance without inventing efficiencies or claiming continuous-box power extrema.
...
v0.49 carries the existing fan pressure/power calculation through the nonlinear uncertainty layer. Every solved corner retains its power evidence; complete studies report evaluated-corner fluid air power and, when explicit fan/motor/VFD efficiencies exist, shaft power, electrical input, and specific fan power. Each reported power extremum also records every tied source corner and its active uncertainty inputs. These remain evaluated-corner diagnostics rather than guaranteed extrema over the continuous uncertainty box because `Q × ΔP` can attain an interior extremum.
