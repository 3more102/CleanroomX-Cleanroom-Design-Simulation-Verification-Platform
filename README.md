...
- Aggregate nonlinear solver-quality evidence across uncertainty corners, including exact worst-metric witness provenance, v0.52 configured residual-tolerance utilization and remaining-margin checks, and v0.56 utilization/remaining-budget checks against the already configured outer-Darcy, inner-Newton, and operating-point iteration limits; pressure-law residual remains a threshold-free raw diagnostic.
...
v0.56 extends the numerical solver audit to iteration budgets that were already explicit study inputs. It records the selected network's inner Newton iteration count and compares worst solved-corner outer-Darcy, inner-Newton, and operating-point iterations with `max_outer_iterations`, `max_newton_iterations`, and `max_operating_iterations`, reporting utilization and remaining iterations with exact witness provenance. These are algorithmic convergence budgets, not engineering acceptance limits.
