...
- Selected operating network-state projection replay diagnostics for v0.91, retaining the accepted solution's canonical node/edge/pressure-power/closure projection and independently comparing it with a fresh selected-airflow solve to report exact JSON-style mismatch paths even when the retained SHA-256 is unchanged.
