...
- Full nonlinear uncertainty fan power-chain evidence, retaining per-corner fluid/shaft/electrical/SFP results and complete-study tie-aware extrema provenance without inventing missing efficiencies or claiming continuous-box power extrema.
...
v0.50 extends that evidence from fluid air power to the complete explicitly configured fan power chain. Each solved corner retains the solver's `power_evidence`; complete studies summarize fluid air power and, when fan/motor/VFD efficiencies are explicitly supplied, shaft power, electrical input, and specific fan power. Every reported power-chain extremum carries tie-aware source-corner provenance. Missing efficiencies remain missing rather than inferred, and indeterminate studies withhold complete power-chain ranges.
