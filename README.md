...
The current release line is **CleanroomX v0.99.0**. It is a direct descendant of clean v0.91 head `cefcbc0b1c83ce041c5ab3da999173e281ddddfb` and preserves the integrated solver/provenance lineage through the desktop application. v0.99 hardens the application registry and desktop worker lifecycle without changing validated engineering solver equations, tolerances, uncertainty semantics, or acceptance criteria.

CI preserves the dedicated v0.95 solver-result-integrity compatibility gate, the v0.94 canonical-provenance, v0.93 supplied-point replay, v0.92 full-bisection projection replay, and v0.91 selected-projection gates, then runs the complete test suite and representative nonlinear loop/uncertainty/dossier smoke checks on Python **3.11, 3.12, and 3.13**. v0.99 validates registry uniqueness/contracts and every declared parser/runner/reporter binding, covers every application workflow end-to-end, and launches the installed desktop entry point under Xvfb on Python 3.13 against the bundled demonstration project.
