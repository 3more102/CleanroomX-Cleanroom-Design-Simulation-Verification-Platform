...
| v0.99 | file-reference path-context provenance, unsaved absolute-reference dossier execution, and Save As result-cache invalidation |

### v0.99 path-context provenance hardening

v0.99 closes two desktop correctness gaps around file-referenced workflows. Engineering dossiers can now run from an unsaved GUI project when every referenced input path is absolute; relative references still require a saved project/base directory and are rejected before execution.

When **Save Project As** changes the project directory, CleanroomX clears all in-memory analysis results before further export. This prevents a cached dossier or consistency result from being presented after the base directory used to resolve relative references has changed. The project inputs remain intact and can be rerun in the new path context.
