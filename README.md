...
- Optional per-sample absolute particle-concentration uncertainty with conservative interval decisions.
...
The v0.13 dossier hashes every referenced input with SHA-256 and preserves component-specific fail, incomplete, indeterminate, not-checked, solved, outside-range, and no-intersection states. Recovery INDETERMINATE results from v0.15 are carried into dossier attention tracking, while HVAC fan-curve duty checks from v0.12 are retained automatically when present. The dossier is a traceability/reporting layer, not cleanroom certification or fan/equipment acceptance.
...
Next milestones are tighter cross-module consistency checks, general looped-network solving research, fan/system control-law studies, broader psychrometric uncertainty, and later a desktop/web UI plus CFD adapters.
