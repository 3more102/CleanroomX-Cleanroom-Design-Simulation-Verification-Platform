...
- Aggregate nonlinear solver-quality evidence across uncertainty corners, including exact worst-metric witness provenance plus v0.52 configured-tolerance utilization and remaining-margin checks for operating-pressure residual, resistance closure, and mass balance; pressure-law residual and iteration counts remain threshold-free diagnostics.
- Optional bounded fan/motor/VFD efficiency uncertainty propagated through solved hydraulic corners as separate power-only endpoint combinations, with explicit `max_power_cases` limiting, provenance, and tie-aware extrema witnesses while leaving hydraulic operating points unchanged.
...
v0.53 optionally adds explicit absolute bounds around supplied fan, motor, and VFD efficiencies. Efficiency endpoint combinations are crossed with solved hydraulic uncertainty corners only for power evidence; they do not trigger a new fan/network operating-point solve because efficiency does not enter the hydraulic model. The workflow validates every efficiency interval inside (0, 1], keeps `power_efficiencies` as the nominal source, limits the combined evidence space with `max_power_cases`, preserves provenance, and reports tie-aware hydraulic-plus-efficiency witnesses for power extrema. These are deterministic evaluated endpoint cases, not statistical uncertainty or guaranteed continuous interior extrema.
