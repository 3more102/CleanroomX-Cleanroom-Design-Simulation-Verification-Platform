...
v0.53 makes the configured solver-tolerance audit directly traceable: every tolerance check carries the tied worst-corner source context, the assessment identifies the maximum utilization ratio and all tied critical residual metrics, and engineering dossiers surface that critical numerical-convergence evidence. This remains solver evidence only, not an equipment or regulatory acceptance criterion.
