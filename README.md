...
- Geometry-derived looped-network resistance from explicit circular/rectangular duct sections, fixed Darcy friction factors, air density, and local-loss coefficients.
...
v0.24 can derive those fixed edge resistances from explicit duct geometry, air density, fixed Darcy friction factors, and local-loss coefficients while preserving the same continuity and pressure-law residual checks. It intentionally does not iterate friction factor with the solved flow.
...
## Solve a geometry-derived looped duct network
...
The v0.24 workflow derives each loop-edge fixed quadratic resistance from explicit circular or rectangular duct geometry, air density, a fixed Darcy friction factor, and explicit local-loss coefficients. It then reuses the v0.23 connected loop solver. Automatic Reynolds/roughness friction is intentionally not applied inside this loop solve. See docs/LOOPED_DUCT_GEOMETRY.md.
