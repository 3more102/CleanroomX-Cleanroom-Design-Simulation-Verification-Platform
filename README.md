...
- Connected looped airflow-network solving for arbitrary graph loops using explicit fixed quadratic edge resistance, signed reverse flow, and nonlinear continuity residuals.
...
The v0.23 solver handles connected meshes with arbitrary loops using explicit fixed edge laws `deltaP = R * Q * abs(Q)` and balanced node injections. It reports signed flow direction, relative node pressures, continuity residuals, and pressure-law residuals. It does not infer duct geometry or couple the loop to a fan curve. See docs/LOOPED_NETWORK_SOLVER.md.
...
Next milestones are geometry-derived resistance and fan coupling for looped networks, closed-loop control studies, and later a desktop/web UI plus CFD adapters.
