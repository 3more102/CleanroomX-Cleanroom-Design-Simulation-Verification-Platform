...
v0.64 maps the configured operating-pressure tolerance and each solved pressure residual through the local fan-minus-system secant gradient into first-order airflow-equivalent numerical diagnostics. The audit also normalizes those equivalents by the active supplied-point bracket span and keeps exact source-corner provenance. These values reuse existing local crossing evidence only; they are not measurement uncertainty, fan-performance uncertainty, interpolation-error bounds, continuous worst-case guarantees, stability criteria, or equipment-acceptance limits.

v0.65 extends the selected-candidate provenance by measuring separation from the solved airflow to every additional discrete candidate. Supplied-point tolerance contacts are treated as point intervals and strict sign-change candidates as their exact supplied-point airflow intervals; the audit retains per-alternative gaps, tied nearest alternatives, overlap state, and the minimum gap across uncertainty corners with source-corner provenance. A zero gap means only that the selected airflow lies inside an alternative discrete candidate interval. The audit does not estimate another continuous root or establish dynamic stability, stall/surge, manufacturer-region, commissioning, certification, or equipment-acceptance criteria.
