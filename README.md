...
- Explicit bounded fan/motor/VFD efficiency uncertainty for nonlinear power evidence, crossed with solved aerodynamic corners without re-solving or changing hydraulic operating points, with an independent combined-power-case limit and tie-aware hydraulic/efficiency witnesses.
...
The current development branch also accepts explicit bounds on supplied fan, motor, and VFD efficiencies. Those bounds affect fan power evidence only: CleanroomX evaluates their endpoint combinations at each already-solved aerodynamic corner, preserves exact efficiency/corner witnesses, and does not infer missing efficiencies or feed efficiency changes back into airflow or pressure.
