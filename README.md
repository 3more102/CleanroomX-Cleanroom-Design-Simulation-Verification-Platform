...
The current release metadata is **CleanroomX v0.99.0**. It is a verified direct descendant of clean v0.91 head `cefcbc0b1c83ce041c5ab3da999173e281ddddfb` and continues through the integrated v0.98 desktop release plus the registry-integrity hardening merged on `main` at `9563507252f204bd8d88fa53f67d4afe2999f040`. v0.99 adds desktop plot/export hardening without replacing validated backend workflows.

CI preserves the dedicated v0.95 solver-result-integrity compatibility gate, the v0.94 canonical-provenance, v0.93 supplied-point replay, v0.92 full-bisection projection replay, and v0.91 selected-projection gates, then runs the complete test suite and representative nonlinear loop/uncertainty/dossier smoke checks on Python **3.11, 3.12, and 3.13**. v0.99 validates registry uniqueness, custom-adapter contracts, and every declared parser/runner/reporter binding, then launches the installed desktop entry point under Xvfb on Python 3.13 against the bundled demonstration project, whose active fan/system analysis exercises real multi-series plot rendering.
...
The desktop GUI is implemented and hardened through v0.96-v0.99. Remaining future work is richer provenance/dependency handling across supplied performance datasets, optional web delivery, and CFD adapters; these are extensions beyond the current desktop release rather than blockers for the supported v0.98 workflows.
