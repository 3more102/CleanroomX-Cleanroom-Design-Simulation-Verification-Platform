...
- Order-invariant network-state fingerprint canonicalization for v0.89, sorting named node, edge, and variable-friction closure collections before hashing so semantically identical solved states remain stable across harmless enumeration-order changes while numeric state changes remain detectable.
