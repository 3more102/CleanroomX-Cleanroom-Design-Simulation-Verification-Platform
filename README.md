...
- Reference-geometry looped-network resistance from one or more explicit duct sections per edge, with manual or reference-flow-resolved Darcy friction and auditable fixed-resistance evidence.
...
v0.25 reference-geometry loop resistance extends that bounded reference-flow approach to connected meshes. Each section's Darcy factor is supplied or resolved once at the edge's declared reference airflow, the resulting series edge resistance is then held fixed during loop balancing, and solved-flow Reynolds/friction iteration remains outside this workflow.
...
## Solve a reference-geometry looped duct network
...
The v0.25 workflow derives every loop-edge fixed quadratic resistance from one or more explicit circular/rectangular duct sections at a declared reference airflow. Darcy friction can be supplied directly or resolved from explicit roughness and kinematic viscosity at that reference flow; it is then held fixed during the connected loop solve. See docs/LOOP_GEOMETRY_RESISTANCE.md.
...
Next milestones are variable-friction looped-network iteration, bounded fan/network coupling, closed-loop control studies, and later a desktop/web UI plus CFD adapters.
