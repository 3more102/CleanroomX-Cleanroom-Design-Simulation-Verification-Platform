...
- Geometry-derived fixed loop resistance from explicit duct sections and reference airflow, reusing manual or automatic Darcy friction while freezing the derived factor during loop balancing.
...
v0.24 geometry-derived loop resistance reuses that automatic-friction calculation only at each edge's declared reference airflow and then freezes the resulting Darcy factor. Reports expose the solved/reference airflow ratio; CleanroomX does not invent a threshold for when a new friction iteration is required.
...
The v0.24 workflow derives each loop edge's fixed quadratic resistance from explicit duct geometry, density, local-loss coefficients, and either a supplied Darcy factor or automatic friction resolved at the edge reference airflow. It then reuses the v0.23 pressure-node solver and reports the resistance derivation plus solved/reference airflow ratio. It does not iterate friction during balancing or couple a fan curve into the mesh. See docs/LOOP_DUCT_NETWORK.md.
