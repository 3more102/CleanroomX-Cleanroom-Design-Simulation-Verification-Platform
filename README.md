...
- Bounded fan-speed ratio sweeps using explicit fan affinity-law scaling of supplied reference curves, with no transformed-curve extrapolation.
...
The v0.19 workflow scales only the supplied reference fan-curve points for explicit user-provided speed ratios using airflow ∝ speed and pressure ∝ speed², reports the cubic speed term only as a homologous-point fan input-power factor, and solves each transformed curve against the explicit system curve without extrapolation. It does not infer acceptable fan/VFD speed limits, efficiency maps, or the motor input power of the newly solved operating point. See docs/FAN_SPEED_STUDY.md.
