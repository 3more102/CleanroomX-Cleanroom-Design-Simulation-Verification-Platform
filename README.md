...
The current release line is **CleanroomX v0.102.1**, completing the synchronized 2D/3D spatial-design workspace on top of the verified Release 2 engineering baseline. It uses one canonical spatial model shared by the 2D editor and 3D viewer, floor/opening metadata, pressure-cascade overlays with observed Δp pass/fail/unavailable state in both views, provenance-backed geometry-to-analysis synchronization, spatial integrity checks, and deterministic layout metrics while preserving the validated solver/provenance backend. Published `v0.100.0`, `v0.101.0`, and `v0.102.0` tags remain immutable at their original validated commits.
