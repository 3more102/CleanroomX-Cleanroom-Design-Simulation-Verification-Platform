...
- Independent nonlinear bisection residual replay for v0.81, reconstructing the active low/high/midpoint airflow states from the selected supplied fan segment and retained L/H/T chain, freshly re-solving the variable-friction loop and fan pressure at each replayed state, and checking every retained residual against that independent evaluation with exact corner/worst-error provenance.
- Independent fan/system residual replay for v0.81, reconstructing each retained bisection low/high/midpoint state from the selected supplied segment, freshly re-solving the nonlinear variable-friction network, and comparing retained residuals against the recomputed fan-minus-system pressure with exact uncertainty-corner and worst-error provenance.
