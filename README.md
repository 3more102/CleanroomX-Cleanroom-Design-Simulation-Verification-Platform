...
- Local pressure-to-airflow root-resolution evidence that maps the configured operating-pressure tolerance and observed solved residual through the same local residual slope, reporting airflow-equivalent numerical resolution and active-segment fractions without presenting them as physical uncertainty or rigorous error bounds.
...
v0.64 adds a local pressure-to-airflow numerical-resolution audit for solved nonlinear uncertainty corners. It reuses the configured operating-pressure tolerance, observed final pressure residual, and v0.60 supplied-bracket residual slope to compute local linearized airflow equivalents and their fractions of the active interpolation segment. Aggregate evidence preserves complete/partial coverage and tied worst-corner provenance. These values are numerical-resolution indicators only; they are not rigorous root-error bounds, interpolation-error estimates, physical uncertainty, dynamic-stability margins, stall/surge limits, manufacturer operating regions, or equipment-acceptance criteria.
