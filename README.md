...
- Independent fan/system residual replay for v0.80, re-solving the loop at every exact supplied-segment bisection low/high/midpoint state, checking each retained fan-minus-system residual against the recomputed value, and independently validating each L/H/T decision against the recomputed midpoint residual, with exact uncertainty-corner violation and worst-error provenance.
