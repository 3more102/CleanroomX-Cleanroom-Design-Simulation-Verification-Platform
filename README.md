...
The v0.17 dossier hashes every referenced input with SHA-256 and covers the current fan workflows, including v0.14 reference-flow fan/duct studies and v0.16 fan-driven passive parallel networks, alongside v0.15 psychrometric uncertainty and the earlier analyses. It preserves component-specific fail, incomplete, indeterminate, not-checked, solved, outside-range, no-intersection, and unresolved-traceability states. The dossier is a traceability/reporting layer, not cleanroom certification or fan/equipment acceptance.
