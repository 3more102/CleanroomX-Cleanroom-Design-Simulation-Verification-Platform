...
- Optional bounded fan/motor/VFD efficiency uncertainty propagated through the solved hydraulic corner set as separate power-only endpoint combinations, with explicit case limits, provenance, and tie-aware witnesses while leaving hydraulic operating points unchanged.
...
v0.52 optionally adds explicit absolute bounds around supplied fan, motor, and VFD efficiencies. Efficiency endpoint combinations are crossed with solved hydraulic corners only for power evidence; they do not trigger a new duct/fan operating-point solve because efficiency does not enter the hydraulic model. The workflow validates every efficiency bound inside (0, 1], limits the combined power-case space with `max_power_cases`, preserves provenance, and reports exact hydraulic-plus-efficiency witnesses for power extrema. These are deterministic evaluated endpoint cases, not statistical uncertainty or guaranteed continuous interior extrema.
