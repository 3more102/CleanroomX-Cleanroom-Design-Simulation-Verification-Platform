...
- Deterministic fan/loop-network uncertainty corner analysis over user-supplied fixed-pressure and named edge-resistance bounds, with bounded no-extrapolation fan solving.
...
v0.29 fan/loop-network uncertainty evaluates explicit lower/upper bounds on fixed pressure and named loop-edge fixed resistances around the v0.26 fan coupling. It solves every unique configured corner, withholds a complete operating-point envelope when any corner falls outside the supplied fan curve, and caps combinatorial expansion with an explicit maximum corner count.
...
Next milestones are engineering-dossier integration for fan/loop uncertainty, followed by a desktop/web UI plus CFD adapters.
