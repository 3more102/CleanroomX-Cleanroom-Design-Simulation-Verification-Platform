...
- Signed-zero-stable network-state fingerprint canonicalization for v0.91, recursively normalizing floating `-0.0` to `0.0` before hashing so representation-only zero-sign differences cannot create false replay mismatches while nonzero state changes remain detectable.
