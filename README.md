...
- Fan-driven passive parallel-network solving by deriving an equivalent quadratic resistance, finding the bounded fan/system operating point, and redistributing the solved airflow across branches.
...
The v0.14 workflow derives the equivalent fixed-resistance curve of passive common-pressure-node branches, combines it with an explicit fixed-pressure term, solves the bounded fan/system intersection, and redistributes the operating airflow across the original branches. It reports mass- and equal-pressure residuals and does not extrapolate supplied fan data. This differs from the v0.12 HVAC design-duty check, which evaluates one required duty point rather than solving an operating point. See docs/FAN_NETWORK_INTEGRATION.md.
