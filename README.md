...
v0.72 retains the complete successful bounded-bisection midpoint decision trace. Each step preserves the active signed-residual bracket, arithmetic midpoint, signed endpoint/midpoint residuals, normalized width, and exact endpoint-replacement or pressure-tolerance acceptance decision. The uncertainty aggregate audits trace coverage and invariants across solved corners and the dossier surfaces the same evidence. Direct supplied-point contacts remain trace-free. This is deterministic numerical implementation provenance only; it is not physical uncertainty, an interpolation-error bound, a continuous-root guarantee, stability/stall/surge evidence, or an equipment-acceptance criterion.
