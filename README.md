...
The current release is **CleanroomX v0.99.0**. It is a verified direct descendant of clean v0.91 head `cefcbc0b1c83ce041c5ab3da999173e281ddddfb` and continues the integrated solver/provenance and desktop-application line without replacing validated backend workflows. v0.99 builds directly on the integrated v0.98 desktop release and the merged registry-integrity hardening at `9563507252f204bd8d88fa53f67d4afe2999f040`.

CI preserves the dedicated v0.95 solver-result-integrity compatibility gate, the v0.94 canonical-provenance, v0.93 supplied-point replay, v0.92 full-bisection projection replay, and v0.91 selected-projection gates, then runs a focused v0.99 application/registry/lifecycle gate, the complete test suite, and representative nonlinear loop/uncertainty/dossier smoke checks on Python **3.11, 3.12, and 3.13**. v0.99 validates catalog uniqueness, direct parser/runner contracts, custom adapters, and every declared application binding, and launches the installed desktop entry point under Xvfb on Python 3.13 against the bundled demonstration project.
...
| v0.99 | registry uniqueness/custom-adapter integrity, structured readiness diagnostics, and synchronized release gates |
