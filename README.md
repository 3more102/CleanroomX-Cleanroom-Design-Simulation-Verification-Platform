...
- Evaluated nonlinear uncertainty power evidence that retains each solved corner's pressure/power record and reports fluid-air, shaft, electrical-input, and specific-fan-power ranges only when the required explicit efficiencies exist, with exact tie-aware source-corner attribution and no continuous-box extremum claim.
...
v0.49 retains the nonlinear solver's full power evidence at every solved uncertainty corner. Complete studies report evaluated-corner ranges for fluid air power and, only from explicitly supplied fan/motor/VFD efficiencies, shaft power, electrical input, and specific fan power. Each available lower/upper power range also carries tie-aware source-corner attribution. These values remain evaluated-corner diagnostics: CleanroomX does not claim that endpoint/corner evaluation proves the continuous-box extrema of Q×ΔP or derived power.
