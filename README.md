...
The v0.13 workflow derives the equivalent fixed-resistance curve of passive common-pressure-node branches, intersects that curve plus an explicit fixed-pressure term with the supplied fan data, and then redistributes the operating airflow across the original branches. It preserves the no-extrapolation boundary and reports mass- and pressure-balance residuals. This is different from the v0.12 HVAC design-duty check, which evaluates one required duty point rather than solving a system operating point. See docs/FAN_NETWORK_INTEGRATION.md.
