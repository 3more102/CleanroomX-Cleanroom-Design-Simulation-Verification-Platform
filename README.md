...
- Deterministic psychrometric-state uncertainty envelopes for dry-bulb temperature, relative humidity, pressure, vapor pressure, humidity ratio, enthalpy, specific volume, dew point, and moist-air specific heat.
...
The uncertainty workflows use deterministic user-supplied input intervals. They are not statistical measurement-uncertainty budgets, do not invent tolerances or acceptance limits, and do not replace calibration records, project qualification procedures, or project/regulatory conformity decision rules. In v0.15, psychrometric state uncertainty can be evaluated independently and can also be propagated through the thermal-uncertainty workflow by enumerating the configured room/outdoor air-state interval corners. This remains bounded screening, not hourly load simulation or equipment selection.
...
The v0.15 workflow evaluates every unique corner of user-supplied dry-bulb, relative-humidity, and total-pressure intervals with the existing CleanroomX psychrometric equations. The same uncertain air-state model is also accepted by thermal-uncertainty analysis, which propagates those states into makeup-air load and sensible-airflow envelopes without assuming probability distributions or covariance.

See docs/PSYCHROMETRIC_UNCERTAINTY.md and docs/THERMAL_UNCERTAINTY.md.
