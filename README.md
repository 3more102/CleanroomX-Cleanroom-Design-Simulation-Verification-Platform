...
## Sweep explicit damper states on a fan-driven loop\n\n    cleanroomx-fan-damper examples/fan_loop_damper_demo.json\n\nJSON output:\n\n    cleanroomx-fan-damper examples/fan_loop_damper_demo.json --format json\n\nWrite a Markdown report:\n\n    cleanroomx-fan-damper examples/fan_loop_damper_demo.json --output fan-damper-report.md\n\nThe v0.27 workflow applies explicit user-supplied additive quadratic resistance to selected loop edges for each named damper state, then re-solves the bounded fan/network operating point and full mesh. It does not infer a resistance characteristic from damper position. See docs/FAN_LOOP_DAMPER_STUDY.md.\n\n## Solve a fan/system operating point
...
Next milestones are fan/loop uncertainty integration, controlled target-flow studies using explicitly supplied device characteristics, and later a desktop/web UI plus CFD adapters.
