...
- Bounded fan/system operating-point solving from explicit piecewise-linear fan static-pressure data and a fixed-plus-quadratic system curve, with no fan-curve extrapolation.
...
The HVAC module is also a preliminary engineering model. The v0.8 branch-flow solver propagates fixed terminal demands through a directed tree by mass continuity, the v0.9 standalone parallel-flow solver distributes a specified total flow across simple paths sharing the same pressure nodes under fixed R·Q² resistance assumptions, and v0.10 intersects explicit fan static-pressure data with an explicit fixed-plus-quadratic system curve. These are bounded steady-state models; CleanroomX does not infer arbitrary looped-network flows, missing fan data, fan-curve extrapolation, variable friction factors, damper positions, leakage, system effect, acoustics, controls, or commissioning acceptance. It does not replace detailed coil selection, weather/load modeling, duct design, manufacturer fan selection, CFD, commissioning, certification, or qualified HVAC/cleanroom engineering review.
...
The v0.10 solver uses piecewise-linear interpolation only inside the supplied fan static-pressure curve and intersects it with an explicit system curve of the form P_fixed + R·Q². It reports no_intersection instead of extrapolating fan data. See docs/FAN_OPERATING_POINT.md.
...
Next milestones are richer engineering reports, uncertainty propagation into thermal and recovery acceptance workflows, general looped-network and multiple-fan solving research, and later a desktop/web UI plus CFD adapters.
