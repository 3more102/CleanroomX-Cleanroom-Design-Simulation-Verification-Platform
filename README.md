...
The current integrated release is **CleanroomX v0.99.0** on `main`. It builds directly on the verified v0.98 desktop release and the registry-integrity hardening integrated on the same mainline. v0.99 adds structural application-registry validation and synchronizes release metadata without changing validated solver equations, numerical tolerances, uncertainty semantics, or engineering acceptance rules.

CI preserves the dedicated v0.95 solver-result-integrity compatibility gate, the v0.94 canonical-provenance, v0.93 supplied-point replay, v0.92 full-bisection projection replay, and v0.91 selected-projection gates, then runs the complete test suite and representative nonlinear loop/uncertainty/dossier smoke checks on Python **3.11, 3.12, and 3.13**. v0.99 additionally validates application-catalog uniqueness, standard parser/runner contracts, the custom consistency/dossier adapter contract, every declared callable binding, and the installed desktop entry point under Xvfb on Python 3.13.
...
| v0.99 | structural application-registry integrity, auditable headless readiness metadata, and final release closure |

### v0.99 desktop application

v0.99 retains the completed user-operable Tkinter application over the existing validated CleanroomX backends and strengthens its registry/readiness contract. Duplicate workflow keys are rejected; ordinary workflows must keep parser and runner bindings; `consistency` and `dossier` remain explicit custom adapters; and every declared parser, runner, and reporter target must resolve to a callable. It provides project creation/open/save, stable strict-JSON `.cleanroomx.json` persistence with migration of supported legacy shapes, unsaved-change protection with a visible dirty marker, analysis creation/rename/removal, per-analysis result restoration, structured JSON input inspection with engineering-unit hints, file import/export, backend validation, guarded non-blocking analysis execution, result/diagnostic/report views, fan-curve plotting where supported, and JSON/Markdown export. The headless `cleanroomx-gui --check` path validates catalog uniqueness, standard/custom adapter contracts, and every declared parser/runner/reporter binding; application regressions execute every workflow exposed by the desktop catalog end-to-end through the shared application service.
