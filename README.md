...
The v0.13 dossier hashes every referenced input with SHA-256 and preserves component-specific fail, incomplete, indeterminate, not-checked, solved, outside-range, and no-intersection states. Recovery INDETERMINATE results from v0.17 are carried into dossier attention tracking, while HVAC fan-curve duty checks from v0.12 remain included when present. The dossier is a traceability/reporting layer, not cleanroom certification or fan/equipment acceptance.
