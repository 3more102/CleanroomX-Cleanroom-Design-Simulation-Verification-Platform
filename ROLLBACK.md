...
If a migrated project must be rolled back to its original legacy representation, restore the archived original file. CleanroomX does not synthesize a reverse migration back to historical legacy shapes.

## Result rollback

Generated JSON/Markdown results are derived artifacts. If the application version changes, regenerate results from the preserved source inputs using the selected validated version rather than editing generated evidence by hand.
