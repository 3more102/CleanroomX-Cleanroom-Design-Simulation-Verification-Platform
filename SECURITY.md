...
CleanroomX v0.99.0 is a local Python desktop/CLI engineering application. It is not a network service, authentication system, secret store, or sandbox for hostile code. The package declares no third-party runtime dependencies; development/test installation adds pytest.
...
Application workflow bindings are fixed in `src/cleanroomx/application.py`; project files select a declared analysis kind rather than arbitrary Python modules or function names. Registry validation rejects duplicate workflow keys, malformed direct/custom adapter contracts, and unresolvable/non-callable bindings.
...
Automated regression coverage is not a formal independent security audit, penetration test, or hostile-input sandbox certification. Security-sensitive deployment should add the organization-specific controls required by its environment.
