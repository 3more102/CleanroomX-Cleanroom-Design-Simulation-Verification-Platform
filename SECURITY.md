...
CleanroomX v0.99.0 is a local Python desktop/CLI engineering application. It is not a network service, authentication system, secret store, or sandbox for hostile code. The Python package declares no third-party runtime dependencies; development/test installation adds pytest.
...
Application workflow bindings are fixed in `src/cleanroomx/application.py`; project files select a declared analysis kind rather than arbitrary Python modules or function names. Registry integrity is validated before ordinary desktop startup.
