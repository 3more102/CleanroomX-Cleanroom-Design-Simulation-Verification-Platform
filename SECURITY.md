...
Desktop project and analysis inputs use strict JSON. Project loading rejects malformed JSON and non-finite constants such as `NaN` and `Infinity`. Standalone engineering-study files and dossier manifests use the shared `src/cleanroomx/jsonio.py` boundary and reject `NaN`, `Infinity`, and `-Infinity` before model construction. CLI JSON output also rejects non-finite result values instead of emitting non-standard JSON tokens. Project schema, version, analysis kinds, ids, and active-analysis references are validated before use.
