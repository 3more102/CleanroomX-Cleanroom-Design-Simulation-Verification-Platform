...
Desktop project and analysis inputs use strict JSON. Project loading rejects malformed JSON and non-finite constants such as `NaN` and `Infinity`. Project files are limited to 64 MiB of raw UTF-8 JSON; the limit is enforced before project hashing/parsing and again on the bounded read, and CleanroomX refuses to save a project larger than the same limit. Invalid UTF-8 project bytes fail closed as a project-format error. Project schema, version, analysis kinds, ids, and active-analysis references are validated before use.
