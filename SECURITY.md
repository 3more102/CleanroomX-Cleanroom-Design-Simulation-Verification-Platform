...
Desktop project and analysis inputs use strict JSON. Project loading rejects malformed JSON and non-finite constants such as `NaN` and `Infinity`. Project files are limited to 64 MiB of raw UTF-8 JSON, including the project member embedded in portable project bundles and the exact project bytes embedded in saved revision artifacts. The limit is enforced before project hashing/parsing and again on bounded reads; revision envelopes are separately bounded to the maximum Base64 payload, one additional project-size allowance for duplicated source strings, and a fixed envelope allowance. CleanroomX refuses to save a project larger than the same project limit. Invalid UTF-8 project bytes fail closed as a project-format error. Project schema, version, analysis kinds, ids, and active-analysis references are validated before use.
