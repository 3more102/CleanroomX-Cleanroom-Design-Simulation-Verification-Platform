...
All file-based engineering JSON entry points use the shared strict JSON reader. Non-standard numeric constants (`NaN`, `Infinity`, and `-Infinity`) and floating-point literals that overflow Python's finite range are rejected before workflow-specific parsing or solver execution, so external JSON cannot introduce non-finite values through decoder behavior.
