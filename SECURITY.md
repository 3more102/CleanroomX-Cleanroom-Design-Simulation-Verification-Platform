...
Successful application runs record a canonical SHA-256 identity for the submitted input. File-backed consistency and dossier workflows record before/after SHA-256 plus byte-size evidence for referenced files. Before backend execution, each referenced engineering file is copied into a private temporary snapshot and the copied SHA-256/size must match the stable pre-run fingerprint; the backend reads the snapshot rather than reopening the original path, and the snapshot is fingerprinted again after backend execution before its result is accepted. The original references are checked again after report construction, and snapshot-storage failures stop before analysis execution. Temporary execution paths are not retained in dossier evidence. These hashes are integrity/provenance evidence, not authentication or a digital signature.
