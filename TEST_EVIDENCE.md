...
PR **#434 — Add PATH-independent Windows launcher for CleanroomX 2D/3D** completed the final repository-checkout usability gap after the Release 2 code consolidation.
...
- Windows `windows-latest` launcher smoke: **success** for both `start-cleanroomx.ps1 --check` and `start-cleanroomx.cmd --check`
