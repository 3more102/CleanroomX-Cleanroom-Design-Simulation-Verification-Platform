...
Project saves are validated before writing and use a same-directory atomic temporary-file replacement. New saves include a canonical SHA-256 integrity record over the complete persisted project envelope excluding the integrity block itself. Save completion also requires read-back through the ordinary strict loader as a stable file revision; on supported POSIX systems the containing directory is fsynced after replacement. The loader rejects integrity mismatches, unsupported integrity contracts, unsupported future schema versions, duplicate analysis ids, invalid active-analysis references, malformed JSON, and non-finite JSON constants such as `NaN` or `Infinity`. Existing schema-version-1 files without an integrity record remain loadable and receive one when next saved. Supported legacy single-analysis shapes are migrated into the current document model on load.
...
Each artifact contains the recoverable project snapshot, the active raw editor draft, application version, recovery timestamp, project identity, a source-file fingerprint containing path, size, modification time, and SHA-256, and a canonical SHA-256 digest over the complete recovery envelope excluding its own integrity block. A malformed JSON editor draft is preserved as raw text without being promoted into the authoritative project model. Recovery-integrity mismatches are reported as unreadable artifacts rather than being restored. Pre-integrity schema-version-1 recovery artifacts remain readable for compatibility. The recovery scanner reports malformed artifacts explicitly and classifies the source project as unchanged, changed, missing, or newer. This foundation never automatically overwrites a newer project file.
