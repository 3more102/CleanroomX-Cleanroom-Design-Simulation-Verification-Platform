...
Project saves are validated before writing and use an atomic temporary-file replacement with post-write byte verification. Opening or successfully saving a project captures a stable SHA-256-backed file revision. A normal **Save Project** compares the current file with that revision immediately before replacement; if another process/session changed or deleted the file, CleanroomX blocks the overwrite, keeps the working copy dirty/recoverable, and directs the operator to **Save Project As** or reopen the on-disk version. Timestamp-only changes with identical file bytes do not create a false conflict. The loader rejects unsupported future schema versions, duplicate analysis ids, invalid active-analysis references, malformed JSON, invalid UTF-8, and non-finite JSON constants such as `NaN` or `Infinity`. Supported legacy single-analysis shapes are migrated into the current document model on load.
