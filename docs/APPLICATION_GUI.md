...
Newly written recovery artifacts carry a SHA-256 integrity digest over the canonical recovery envelope (excluding the digest field itself). Recovery loading recomputes and verifies that digest before the artifact can be inspected, restored, or discarded, so valid-JSON content changes are surfaced as unreadable recovery issues rather than silently accepted. Recovery Center shows the integrity state explicitly. Older recovery artifacts without an embedded digest remain readable and are labeled **Legacy / no digest** for backward compatibility. The digest is corruption/tamper evidence for the stored artifact; it is not a digital signature and does not prove author identity or authenticity.
