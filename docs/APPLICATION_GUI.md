...
Spatial design edits have a bounded 100-step **Undo/Redo** history. Use the toolbar or **Ctrl+Z / Ctrl+Y**. **Duplicate** or **Ctrl+D** clones the selected room/device with a small grid offset and a new stable ID. Arrow keys nudge the current selection by one configured grid interval when snapping is enabled, or by 0.1 m when snapping is disabled. Selecting a room exposes eight direct resize handles (corners and edge midpoints); dragging a handle resizes the room, respects grid snapping when enabled, preserves the opposite edge, and is recorded as a single undoable edit. With **Align** enabled, dragging a room also magnetically aligns its left/center/right and top/center/bottom anchors to nearby room anchors within an 8-pixel screen tolerance, with live dashed alignment guides; alignment remains part of the same single undoable move. With **Gaps** enabled, selecting a room displays the nearest orthogonal clearance to neighboring rooms on its left, right, top, and bottom wherever the perpendicular room spans overlap; touching rooms are explicitly shown as 0 m and diagonal-only neighbors are ignored. CleanroomX also detects positive-area room-footprint overlaps: the scene summary reports both the conflict count and cumulative overlap area and, with **Conflicts** enabled, each intersection is outlined and crossed in red with its overlap area when space permits. Rooms that merely share an edge or corner are not treated as overlapping. Use **Prev conflict** / **Next conflict** to review overlap pairs bidirectionally in stable order; the **[** and **]** keys provide the same navigation when either spatial canvas has focus. Each review step automatically frames both rooms in the 2D canvas and emphasizes the active overlap in orange while leaving all other conflicts visible. CleanroomX selects the later room in each pair, draws the deterministic proposed repair as a teal dashed ghost footprint with a movement arrow, and reports the move distance plus X/Y delta in the status line before any geometry changes. Assigned-device count is included when those devices will move with the room. Use **Resolve overlap** to commit that same deterministic move for the selected room and its assigned devices as an undoable spatial edit. Use **Resolve all** to clean the whole layout in stable room order: earlier rooms stay fixed, later conflicting rooms move only as needed, assigned devices move with their rooms, and the entire cleanup is recorded as one undoable spatial edit. View-only navigation such as pan/zoom/rotate is not inserted into the edit history.
