...
Spatial room and device identities are persistent model data. CleanroomX reserves every explicit persisted ID before generating repairs, preserves the first valid identity, deterministically repairs missing or duplicate IDs, enforces uniqueness for both rooms and devices, and normalizes device-to-room references before the workspace uses them. If a device reference matched multiple source-room identities before canonicalization—including duplicate explicit IDs or a collision between a legacy name-derived ID and an explicit ID—the spatial validation status keeps an explicit warning for the open-project session so unrelated viewport changes, undo/redo, or other persistence do not silently clear the repair evidence. Reopening or repeatedly normalizing the same legacy/malformed spatial metadata therefore produces the same repaired identities rather than new random IDs. Analysis-derived layouts also allocate deterministic suffixes when room names collide.
