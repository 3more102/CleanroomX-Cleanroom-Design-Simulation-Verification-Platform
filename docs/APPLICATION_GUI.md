...
Project saves are validated before writing and use an atomic temporary-file replacement. The loader rejects unsupported future schema versions, duplicate analysis ids, invalid active-analysis references, malformed JSON, and non-finite JSON constants such as `NaN` or `Infinity`. Supported legacy single-analysis shapes are migrated into the current document model on load. For schema-version-1 files, unknown fields at the document, project-block, and analysis-entry levels are preserved opaquely across load/save cycles so external extension metadata is not silently deleted. CleanroomX does not interpret those fields, and its reserved core fields always remain authoritative.
