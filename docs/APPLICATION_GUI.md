...
For room-verification and multi-room project-verification analyses, **Sync dimensions to active analysis** explicitly copies room dimensions (and an existing observed-pressure field when present) from the spatial model into the analysis JSON. Other engineering fields such as airflow, ACH requirements, particle requirements, and pressure-cascade criteria are preserved. The synchronization validates its complete room-name mapping before applying changes: duplicate or case-ambiguous room names fail closed, and a single-room analysis used with a multi-room spatial layout must match exactly one room by name. A failed synchronization applies no geometry changes. Results for a successful synchronized analysis are invalidated and must be validated/run again.
