...
Existing projects remain schema-version-1 compatible because the spatial document is stored under the existing project metadata block. Layout normalization is deterministic: repeated normalization of the same supported layout produces the same room/device ids and colliding derived room names receive stable suffixes. Newly created GUI rooms/devices receive full UUID identities once, preventing delete/recreate cycles from reusing a prior object's persistent identity. If no spatial metadata exists, CleanroomX can seed a layout from real room geometry found in a verification analysis. Projects with no such geometry remain empty until the operator adds rooms.
