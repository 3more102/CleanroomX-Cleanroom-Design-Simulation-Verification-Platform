...
Diagnostics also identify the exact CleanroomX source revision that produced a run. CleanroomX fingerprints installed package Python source by relative path and exact bytes before and after execution, records the parser/runner/reporter or custom adapter selected by the application registry, and records Python implementation/version, platform identity, byte order, and floating-point radix/mantissa metadata. Unchanged source-tree hashes are reused only while file identity, size, nanosecond mtime, and ctime evidence is unchanged. The source hash is independent of the package version, so editable or unreleased builds that still report the same semantic version remain distinguishable. If the CleanroomX source tree changes during an active run, the result is discarded and the operator must rerun from one stable installation.
