...
All backend outputs are normalized to strict JSON with non-finite values rejected. Successful runs record canonical application-input SHA-256 provenance; consistency/dossier runs also capture before/after SHA-256, byte-size, and nanosecond modification-time evidence for external dependencies. Every run additionally records the exact CleanroomX Python-source-tree SHA-256 before and after execution, source-file count, Python/runtime platform identity, and the configured validation/execution/reporting entry points. If the CleanroomX source tree changes during validation, solving, reporting, diagnostics, plotting, or external-dependency recapture, the result is discarded and must be rerun after the installation is stable. Diagnostics exposes this evidence and **Export Run Bundle JSON** preserves the completed run data.

`cleanroomx-gui --check` also reports the current implementation revision so an installation can be identified before any engineering run. The source-tree digest is location-independent and newline-normalized, but it is an integrity/reproducibility identifier rather than a digital signature or proof that the software is trustworthy. It intentionally covers CleanroomX Python source; in-memory monkeypatching after import and external interpreter/native-library behavior remain outside that digest.
