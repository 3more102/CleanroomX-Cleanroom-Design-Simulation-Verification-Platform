...
The 2D view supports room creation, selection, drag movement with metric grid snapping, property editing and resizing, deletion, zoom, pan, fit-to-view, coordinate feedback, and placement of doors, FFUs, supply points, returns, exhausts, equipment, and sensors. Drag translation is calculated from the immutable pointer/item position captured at gesture start, so the final snapped geometry is deterministic even when the window system coalesces or emits different intermediate motion events. A click/release or drag that returns to the original geometry is treated as a no-op: it does not create undo history, mark the project dirty, or schedule recovery autosave. Spatial model edits have bounded transactional **Undo/Redo**: add, delete, property changes, and a full drag gesture are each one history operation; redo is invalidated by a new divergent edit, and undo/redo restores selection without rewinding the current camera/view state. Toolbar buttons expose the feature, with Ctrl+Z, Ctrl+Y, and Ctrl+Shift+Z available while a spatial canvas has focus. When room pressure is present in a verification input, the layout visualizes only that supplied pressure data; it does not invent pressure values.
