...
Check that the application layer, GUI imports, and every declared parser/runner/reporter binding resolve without opening a window:
...
Regression coverage includes application-catalog/backend reuse, resolution of every declared parser/runner/reporter binding, strict result serialization, relative-file adapters, project round-trip/migration/rejection cases, engineering-unit/path flattening, headless `--check`, and execution of the active demonstration analysis.
