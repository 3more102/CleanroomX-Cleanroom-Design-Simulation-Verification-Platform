...
For room-verification and multi-room project-verification analyses, **Sync dimensions to active analysis** explicitly copies room dimensions (and an existing observed-pressure field when present) from the spatial model into the analysis JSON. Other engineering fields such as airflow, ACH requirements, particle requirements, and pressure-cascade criteria are preserved. The mapping is validated completely before mutation: duplicate/ambiguous room names fail closed, and a single-room analysis opened against a multi-room layout must match exactly one spatial room by name. A failed synchronization applies no geometry changes. Results for a successful synchronized analysis are invalidated and must be validated/run again.

Existing projects remain schema-version-1 compatible because the spatial document is stored under the existing project metadata block. If no spatial metadata exists, CleanroomX can seed a layout from real room geometry found in a verification analysis. Projects with no such geometry remain empty until the operator adds rooms. Existing unique spatial ids are preserved. Missing or duplicate room/device ids in legacy or malformed spatial metadata are repaired deterministically with stable suffixes during normalization, so repeated normalization of identical source data produces identical canonical identities instead of random replacements.
