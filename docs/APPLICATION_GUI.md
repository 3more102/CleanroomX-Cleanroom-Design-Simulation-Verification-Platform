...
Autosave never writes to the open `.cleanroomx.json` path. It writes a versioned `cleanroomx.autosave` recovery envelope in the per-user recovery directory using the same atomic-write primitive as project persistence. Writes run on a single background worker, identical snapshots are suppressed, newer pending edits are coalesced, and history is bounded independently for each project/session pair. Recovery filenames carry a SHA-256-derived session token only for efficient ownership scoping; the recovery envelope remains the authority, and rotation validates its project and session identity before deletion. A concurrent or older CleanroomX session therefore cannot lose its recoveries because another session reaches its own history limit. Unreadable or ownership-mismatched artifacts are retained rather than pruned automatically.
