...
**Analysis → Run History...** labels each retained run as **INPUT MATCH** only when its recorded analysis-input hash matches the live editor draft; otherwise it is explicitly **HISTORICAL INPUT**. This label describes inline input identity only; external dependency evidence remains visible in the retained run provenance. Historical results are displayed in a separate window and never become the current cached result. An operator can export a selected historical run bundle explicitly; the existing current-result export path continues to reject stale results.
