...
Spatial design edits have a bounded 100-step **Undo/Redo** history. Use the toolbar or **Ctrl+Z / Ctrl+Y**. **Duplicate** or **Ctrl+D** clones the selected item with a small grid offset and a new stable ID; duplicating a room also clones every device assigned to that room, preserves each device's relative placement/elevation, and relinks the copies to the new room. Arrow keys nudge the current selection by one configured grid interval when snapping is enabled, or by 0.1 m when snapping is disabled. Selecting a room exposes eight direct resize handles (corners and edge midpoints); dragging a handle resizes the room, respects grid snapping when enabled, preserves the opposite edge, and is recorded as a single undoable edit. Assigned devices preserve their normalized XY position within the room during the resize, so shrinking or expanding a room does not silently strand attached devices outside its footprint. View-only navigation such as pan/zoom/rotate is not inserted into the edit history.
