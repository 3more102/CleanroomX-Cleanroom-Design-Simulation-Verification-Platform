...
Spatial design edits have a bounded 100-step **Undo/Redo** history. Use the toolbar or **Ctrl+Z / Ctrl+Y**. **Duplicate** or **Ctrl+D** clones the selected room/device with a small grid offset and a new stable ID. Arrow keys nudge the current selection by one configured grid interval when snapping is enabled, or by 0.1 m when snapping is disabled. Selecting a room exposes eight direct resize handles (corners and edge midpoints); dragging a handle resizes the room, respects grid snapping when enabled, preserves the opposite edge, and is recorded as a single undoable edit. Devices assigned to that room retain their normalized XY placement during resize, so ceiling devices, equipment, sensors, and boundary-mounted points follow the edited geometry instead of being stranded outside it. The same device-aware geometry remap is applied when room position or dimensions are edited through the Properties panel; ceiling-associated devices also track an edited room height. View-only navigation such as pan/zoom/rotate is not inserted into the edit history.
