...
Project saves are validated before writing and use a verified same-directory atomic commit. CleanroomX encodes exact UTF-8 bytes, flushes and `fsync()`s the staging file, re-reads a stable staging revision and requires the intended SHA-256/byte size **before** replacing the authoritative path, then atomically replaces the destination, `fsync()`s its containing directory where supported, and re-verifies the committed file. Unexpected directory-sync failures raise an explicit durability error; staged or committed byte mismatches raise an explicit verification error, so the desktop does not mark the in-memory project clean after an unverified write. The loader rejects unsupported future schema versions, duplicate analysis ids, invalid active-analysis references, malformed JSON, and non-finite JSON constants such as `NaN` or `Infinity`. Supported legacy single-analysis shapes are migrated into the current document model on load.
...
Autosave never writes to the open `.cleanroomx.json` path. It writes a versioned `cleanroomx.autosave` recovery envelope in the per-user recovery directory using the same staged-and-committed byte-verified atomic primitive as project persistence. Source fingerprints reject path replacement as well as size/mtime mutation during hashing. Writes run on a single background worker, identical snapshots are suppressed, newer pending edits are coalesced, and history is bounded per project identity.
...
7. Export input/result JSON, complete run-bundle JSON, or report Markdown and save the project. Writes use the shared durable verified commit path; filesystem, directory-sync, and post-write verification errors are surfaced in the GUI.
