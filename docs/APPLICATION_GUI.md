...
Live spatial validation uses an adaptive sweep broad phase for room-overlap detection instead of testing every room pair blindly. Exact overlap acceptance still uses the explicit 1e-9 m geometry tolerance and validation findings retain original project room order. The workspace caches validation against validation-relevant model state only, so camera orbit, pan, zoom, selection, pressure-only visualization changes, and unchanged redraws do not rerun geometry validation. Any room/device field used by validation invalidates the cache automatically.
