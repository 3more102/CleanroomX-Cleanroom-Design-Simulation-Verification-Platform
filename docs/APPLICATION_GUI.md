...
For `consistency` and `dossier`, those external dependencies are guarded as engineering run inputs rather than treated as advisory provenance only. CleanroomX fingerprints each referenced file from a stable read, creates a private execution-only copy, and requires the copy's SHA-256 and byte count to match the recorded pre-run revision before the backend can execute. The custom backend then reads only the private snapshots, not the original paths, and CleanroomX re-fingerprints those snapshots after backend execution before accepting the result. CleanroomX fingerprints the originals again after result/report generation; if a source changes, disappears, cannot be copied consistently, or remains unstable while being fingerprinted, the run fails with an actionable error and the result is discarded. If the source is stable but private snapshot storage fails (for example because the temporary filesystem cannot accept the copy), the analysis does not start and the storage failure is reported separately. Diagnostics records `external_dependency_execution_mode = immutable_content_snapshot_v1` plus the exact execution snapshot hash/size. Snapshot files are deleted after the run, and dossier source records retain the user-declared paths rather than temporary execution paths. This prevents transient change-and-revert races from changing the bytes consumed by a run even when the original path later returns to its prior revision; it does not lock files against other programs.
