...
Spatial object identity is canonicalized before use. Valid room/device ids are preserved, while missing or duplicate ids are repaired with deterministic suffixes; explicit ids declared later in the file are reserved so repair cannot steal them. If legacy or malformed metadata makes a device room reference match multiple source rooms—through duplicate explicit ids or a collision between a legacy name-derived id and an explicit id—CleanroomX preserves canonical compatibility behavior and reports an explicit spatial validation warning so the operator can review the assignment. Repeated normalization of the same data produces the same canonical layout.
