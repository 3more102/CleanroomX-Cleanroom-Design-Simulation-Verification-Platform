...
Spatial design data remains stored inside the schema-version-1 metadata block, but it has its own explicit `spatial_layout.version` contract. Project load and save now reject unsupported future spatial versions, duplicate room or device ids, devices that reference missing rooms, non-finite coordinates/pressure/elevation, non-positive room dimensions/grid size, and persisted viewport values outside the supported runtime bounds. The validator does not silently repair persisted corruption. Interactive normalization is still tolerant for in-progress edits, and any generated replacement ids are deterministic so repeated normalization of the same input produces the same model.
