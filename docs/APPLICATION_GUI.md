...
On normal interactive startup, CleanroomX scans the recovery directory before opening a command-line project argument. If recovery data exists, **Recovery Center** lists the project name, exact UTC recovery timestamp, source comparison state, and original source path. **Inspect…** shows the project identity, application version, recovered analysis list, raw editor draft, and a semantic comparison against the current original project. The comparison reports exact changed project paths, matches analyses by stable analysis ID, and ignores file-only serialization differences such as whitespace or runtime application-version text. Missing or unreadable source files are reported as unavailable rather than guessed. Malformed/unreadable recovery artifacts are reported and preserved.
