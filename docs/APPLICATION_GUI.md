...
For room-verification and multi-room project-verification analyses, **Preview + sync to analysis** performs a non-mutating dry run first. The confirmation dialog lists the exact room fields that would change, the current analysis value, the spatial value, matched-room count, and any unmatched-room warnings. Multi-room synchronization remains name-based, but duplicate room names are now treated as an ambiguity and block the sync rather than risking an update to the wrong room. After confirmation, CleanroomX copies room dimensions (and an existing observed-pressure field when present) from the spatial model into the analysis JSON. Other engineering fields such as airflow, ACH requirements, particle requirements, and pressure-cascade criteria are preserved. Results for a synchronized analysis are invalidated and must be validated/run again.
