...
Autosave never writes to the open `.cleanroomx.json` path. It writes a versioned `cleanroomx.autosave` recovery envelope in the per-user recovery directory using the same atomic-write primitive as project persistence. Writes run on a single background worker, identical snapshots are suppressed, newer pending edits are coalesced, and history is bounded per project identity. A completed Future remains coordinator-owned until its completion callback has finalized the active and pending request state, so an edit arriving in the done-before-callback window is queued rather than replacing the tracked write. Retention cleanup is committed only after the completed request is revalidated against the current project epoch. If an explicit save, recovery discard, or project transition invalidates an in-flight write, that stale artifact is deleted and is not allowed to rotate older accepted recovery generations; a filesystem failure while deleting that invalidated artifact is surfaced as an autosave failure with the artifact path retained for diagnosis. If the recovery itself was written but retention cleanup fails, CleanroomX keeps the new recovery artifact and surfaces the cleanup failure instead of silently reporting a fully successful autosave.
