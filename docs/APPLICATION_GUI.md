...
Project saves are validated before writing and use an atomic same-directory temporary-file replacement. When an existing project is opened, CleanroomX records a SHA-256-backed revision from the exact bytes it parsed. Normal **Save** and **Save Project As** back onto that same path compare the current disk revision before replacement; if another process changed, replaced, removed, or moved the file, CleanroomX blocks the overwrite, keeps the local work unsaved, and directs the operator to Save As or reopen the disk version. Successful saves are read back and content-verified before the new revision becomes the save baseline. Serialization is deterministic UTF-8 with LF newlines. The loader rejects unsupported future schema versions, duplicate analysis ids, invalid active-analysis references, malformed JSON, and non-finite JSON constants such as `NaN` or `Infinity`. Supported legacy single-analysis shapes are migrated into the current document model on load.
