...
New recovery artifacts use schema version 2 and contain the recoverable project snapshot, active raw editor draft, application version, recovery timestamp, project identity, source-file fingerprint, and a canonical SHA-256 integrity block. After the atomic write, CleanroomX reopens and verifies the artifact before rotating an older recovery generation. If that immediate verification fails, the new bad artifact is removed best-effort and the previous recovery history is preserved. A structurally valid version-2 artifact later discovered with a payload that no longer matches its digest is rejected from restoration and reported as a preserved scan issue. Existing version-1 artifacts remain readable and are labeled **Legacy artifact — no embedded checksum**. The SHA-256 is corruption evidence, not a digital signature or proof of source authenticity.

A malformed JSON editor draft is preserved as raw text without being promoted into the authoritative project model. The recovery scanner reports malformed or integrity-invalid artifacts explicitly and classifies the source project as unchanged, changed, missing, or newer. This foundation never automatically overwrites a newer project file.
