...
Spatial object IDs are persistent model identity, not display labels. Missing or duplicate legacy room/device IDs are repaired deterministically during normalization, and the normalized result is idempotent across repeated loads. When a legacy file contains duplicate room IDs, an existing device reference keeps the historical first-match target while the later room receives a collision-free ID. New workspace objects use the same collision-safe allocator. This prevents one selection/delete operation from ambiguously targeting multiple spatial objects while preserving the project schema and existing room-reference semantics.
