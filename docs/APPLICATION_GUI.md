...
Diagnostics now also identify the exact software revision that produced a run. CleanroomX fingerprints all installed package Python source files by relative path and exact bytes before and after execution, records the parser/runner/reporter or custom adapter selected by the application registry, and records Python implementation/version, platform identity, byte order, and floating-point radix/mantissa metadata. The source hash is independent of the package version, so editable or unreleased builds that still report the same semantic version remain distinguishable. If the CleanroomX source tree changes during an active run, the result is discarded and the operator is told to restart from one stable installation before rerunning.
