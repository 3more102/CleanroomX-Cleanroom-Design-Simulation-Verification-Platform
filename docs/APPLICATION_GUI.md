...
Each artifact contains the recoverable project snapshot, the active raw editor draft, application version, recovery timestamp, project identity, and a source-file fingerprint containing path, size, modification time, and SHA-256. New schema-v2 artifacts also store a deterministic SHA-256 checksum over the complete semantic recovery payload. CleanroomX verifies that checksum before scan, inspection, restore, or validated discard; a parseable but modified artifact is preserved as a recovery issue instead of being trusted. Existing schema-v1 recovery artifacts remain readable and are labeled legacy/unverified because they predate end-to-end checksums. This checksum is corruption evidence, not cryptographic authentication or signer identity. A malformed JSON editor draft is preserved as raw text without being promoted into the authoritative project model. The recovery scanner reports malformed artifacts explicitly and classifies the source project as unchanged, changed, missing, or newer. This foundation never automatically overwrites a newer project file.
