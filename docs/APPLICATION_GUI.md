...
The spatial workspace also exposes an operator scene summary with room/device counts, total cleanroom footprint, enclosed volume, available pressure range, and unassigned-device warnings. Grid snapping can be toggled independently from grid visibility. Dragged devices are reassigned to the room beneath their new position, and ceiling-mounted devices follow the destination room height. Doors assigned inside a room snap deterministically to the nearest room wall and render as wall-oriented door markers in the 2D plan instead of generic point symbols; the existing schema remains unchanged. Moving a room also carries its assigned devices so room/device geometry does not silently separate. The 3D perspective includes a floor extent, X/Y orientation axes, and pressure-range cue so the spatial model remains readable while rotating or zooming. Both canvases support **F** to fit and **Delete** to remove the current selection after the canvas has focus.

Spatial design edits have a bounded 100-step **Undo/Redo** history. Use the toolbar or **Ctrl+Z / Ctrl+Y**. **Duplicate** or **Ctrl+D** clones the selected room/device with a small grid offset and a new stable ID. Arrow keys nudge the current selection by one configured grid interval when snapping is enabled, or by 0.1 m when snapping is disabled. Selecting a room exposes eight direct resize handles (corners and edge midpoints); dragging a handle resizes the room, respects grid snapping when enabled, preserves the opposite edge, and is recorded as a single undoable edit. View-only navigation such as pan/zoom/rotate is not inserted into the edit history.
