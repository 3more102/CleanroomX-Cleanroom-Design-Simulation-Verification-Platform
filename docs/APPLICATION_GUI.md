...
Spatial object identity is canonicalized before use. Valid room/device ids are preserved, while missing or duplicate ids are repaired with deterministic suffixes; explicit ids declared later in the file are reserved so repair cannot steal them. If legacy or malformed metadata contains duplicated room ids and a device refers to that duplicated value, CleanroomX keeps the historical first-room binding for compatibility and reports an explicit spatial validation warning so the operator can review the assignment. Repeated normalization of the same data produces the same canonical layout.
