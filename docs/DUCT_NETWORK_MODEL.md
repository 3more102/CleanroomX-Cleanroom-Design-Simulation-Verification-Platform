...
CleanroomX v0.6 adds a transparent, path-based duct pressure-loss calculation for preliminary cleanroom HVAC design. v0.6.1 hardens numeric validation so non-finite inputs are rejected before analysis. v0.20 adds an optional Reynolds/roughness-based Darcy friction-factor calculation while preserving the original explicit-factor workflow.
...
## v0.20 automatic friction-factor option
...
- The passive parallel-flow solvers remain fixed-resistance models and continue to require explicit friction factors; v0.20 does not implement nonlinear variable-friction pressure balancing.
