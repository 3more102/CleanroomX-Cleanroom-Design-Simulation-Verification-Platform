...
Paths are resolved relative to the manifest file. Existing manifests that omit the v0.12 fields remain valid.
...
- `attention_required`: one or more configured checks failed, an HVAC fan-curve duty check fails or lies outside the supplied curve range, a recovery test is incomplete, an uncertainty result is indeterminate, or a fan/system study has no intersection inside the supplied fan-curve range;
...
A solved fan operating point or a passing HVAC fan-curve duty check is reported as completed engineering screening, not as equipment acceptance. HVAC calculations remain preliminary screening.
