...
A recovery result that is indeterminate because its supplied concentration-uncertainty interval overlaps the target is preserved as an attention item rather than being promoted to pass or collapsed into fail. Solved standalone, reference-flow fan/duct, and passive parallel-network operating points are reported as bounded engineering screening, not equipment acceptance. A complete fan/system uncertainty envelope is reported only when every bounded system corner intersects the supplied fan curve; otherwise the analysis remains indeterminate. A completed psychrometric-state envelope is likewise screening rather than a conformity decision. Missing psychrometric or fan-system provenance is tracked as unresolved traceability rather than a numerical failure. HVAC calculations remain preliminary screening.
