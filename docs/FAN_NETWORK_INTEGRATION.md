...
CleanroomX v0.14 couples the bounded fan-curve operating-point solver from v0.11 to the passive common-pressure-node parallel-path model from v0.9.
...
The v0.12 HVAC fan-curve duty check answers whether supplied fan data can meet one explicit required duty point. The standalone v0.11 solver intersects a user-entered fixed-plus-quadratic system curve with fan data. This v0.14 workflow instead derives the quadratic term from the modeled passive parallel branches before solving the operating point and branch flows.
