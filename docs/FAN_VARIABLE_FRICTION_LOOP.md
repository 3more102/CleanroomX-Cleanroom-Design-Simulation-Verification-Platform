...
v0.91 applies the same field-level projection replay to the accepted selected operating solution. The selected replay retains the canonical network-state projection alongside its SHA-256, independently re-solves the exact retained selected airflow, and reports deterministic JSON-style mismatch paths plus recorded/recomputed values, type evidence, numerical absolute errors when meaningful, and tied per-field worst-error witnesses when the projection differs. This catches and localizes selected-state projection corruption even when the retained selected airflow, scalar fan/loop/system/residual evidence, and stored SHA-256 remain unchanged. Projection replay is deterministic numerical/provenance verification only: it is not cleanroom certification, CFD validation, fan acceptance, commissioning evidence, proof of global root uniqueness, stall/surge analysis, or physical uncertainty quantification.

v0.92 extends the same shared canonical projection comparator across the complete retained bounded-bisection trace. Every low, midpoint, and high state now retains its canonical projection alongside the v0.87 SHA-256 fingerprint, while replay reconstructs the exact search-origin airflow sequence and independently re-solves each state. The audit separates fingerprint identity from projection equality, preserves every deterministic JSON-style difference with iteration and low/midpoint/high position provenance, records retained/recomputed values and types, reports numerical absolute errors where meaningful, retains tied per-field worst witnesses, and explicitly reports incomplete state-position coverage rather than treating unchecked states as consistent. No candidate discovery, interpolation, root-selection, tolerance, iteration-budget, convergence, power, or engineering-acceptance semantics are changed. This is deterministic numerical/provenance verification only; it is not cleanroom certification, CFD validation, fan acceptance, commissioning evidence, proof of global root uniqueness or physical stability, stall/surge analysis, manufacturer operating-envelope validation, physical uncertainty quantification, or statistical confidence analysis.

v0.93 extends internal network-state replay to the supplied fan-curve sample set that precedes operating-point candidate selection. Each successfully evaluated supplied point now retains the same canonical SHA-256 and full projection used by terminal, selected, and full-bisection replay. A fresh nonlinear solve at the exact supplied airflow independently checks both forms, while projection mismatches retain point index, deterministic JSON path, recorded/recomputed values and types, numerical absolute errors when meaningful, and tied per-field worst witnesses. The audit explicitly distinguishes complete sampled-point coverage, partial evaluation prefixes, missing retained evidence, independent replay failures, hash corruption, and projection corruption. Complete no-intersection results therefore remain internally auditable, while pre-failure partial coverage is not presented as complete. Candidate discovery, fan interpolation/no-extrapolation, root priority, tolerances, nonlinear convergence, power calculations, and engineering acceptance are unchanged.

v0.94 hardens the shared canonical network-result representation used by supplied-point, full-trace, terminal, and selected replay. Floating signed zero is normalized, named node/edge/closure collections are sorted by name, and network/status/reference-node identity, inner Newton iterations, mass-balance tolerance, variable-friction convergence/configuration, and chronological outer-iteration history are included. Iteration-history order remains significant by design. The canonicalization change affects provenance identity and mismatch localization only; it does not alter the nonlinear equations, fan/root search, tolerances, no-extrapolation policy, or engineering acceptance.
