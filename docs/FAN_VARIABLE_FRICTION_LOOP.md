...
v0.80 independently replays the fan/system residual values themselves. Starting from the exact selected supplied fan-curve segment, it reconstructs each bisection low/high/midpoint airflow from the decision sequence, recomputes fan pressure from the same bounded interpolation segment, fully re-solves the variable-friction loop at that airflow, and compares the resulting fan-minus-system pressure with the retained residual. It also derives the expected L/H/T action from the independently recomputed midpoint residual and configured pressure tolerance, rather than trusting the retained residual. This catches self-consistent corruption of stored residual values that can survive sign, transition, and recorded-residual decision-semantic checks. The replay tolerance is numerical implementation tolerance only and does not define an engineering acceptance band.
