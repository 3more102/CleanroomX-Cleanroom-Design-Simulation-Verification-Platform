...
v0.92 normalizes IEEE-754 signed zero recursively throughout that canonical projection before either field-by-field comparison or SHA-256 serialization. A retained `-0.0` and `0.0` therefore identify the same implementation state in both v0.91 projection replay and the existing fingerprint replay; every nonzero retained value remains unchanged and hash-sensitive. Replay evidence reports the centralized v3 signed-zero-normalized canonicalization identifier.
