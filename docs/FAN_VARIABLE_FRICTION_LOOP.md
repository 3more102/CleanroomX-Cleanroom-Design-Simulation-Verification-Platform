...
For retained bounded-bisection traces, v0.80 records midpoint fan pressure and total system pressure, checks complete operand coverage, independently verifies the stored fan-minus-system residual identity, and checks the recorded L/H/T decision against that recomputed residual. These are numerical provenance checks only, not physical uncertainty or equipment-acceptance margins.
