...
v0.81 adds an independent nonlinear residual replay. Starting from the selected supplied fan-curve segment, the audit reconstructs the active low/high/midpoint airflow states from the retained L/H/T decision chain, freshly re-solves the variable-friction network and fan interpolation at each replayed airflow, and compares every retained fan-minus-system residual with that independent evaluation. This catches self-consistent retained-residual corruption that arithmetic pressure-state identities or decision-chain replay alone can miss. The replay is still implementation provenance using the same configured model inputs; it does not establish model-form accuracy or physical operating uncertainty.
