...
CleanroomX v0.39 combines the existing bounded fan-speed transformation with the current nonlinear fan/variable-friction uncertainty engine.

## What is evaluated

For every explicit fan speed ratio, CleanroomX scales the supplied reference fan-curve points using the existing affinity-law transform: airflow scales with speed and pressure scales with speed squared. It then evaluates every configured lower/upper nonlinear uncertainty corner at that speed.

The supported bounded inputs are inherited directly from the current nonlinear uncertainty workflow:

- fixed system pressure;
- selected automatic-friction duct local-loss coefficients `K`;
- selected absolute roughness values;
- selected kinematic-viscosity values; and
- selected air-density values.
