...
v0.75 extends the same trace to iteration-limit outcomes. Those corners retain every completed L/H midpoint evaluation, remain explicitly `non_converged`, contain no fabricated terminal T decision, and replay the final L/H replacement into the retained remaining signed-residual bracket. The aggregate separates solved and iteration-limit trace coverage and reports exact terminal-outcome and final-replay violation indices together with maximum trace-length provenance. This is numerical implementation evidence only; it does not establish physical airflow uncertainty, interpolation error, continuous-root uniqueness/stability, manufacturer operating limits, commissioning/certification status, or equipment acceptance.
