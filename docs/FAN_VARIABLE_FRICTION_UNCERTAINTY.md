...
v0.91 propagates terminal network-state projection replay across solved and iteration-limit uncertainty corners using the same order-invariant v2 canonical projection. The aggregate records projection-consistent and violating corner indices, exact low/high endpoint positions, and deterministic field mismatch paths while preserving the separate SHA-256 fingerprint replay evidence. These are implementation-provenance diagnostics only; they do not modify corner selection, nonlinear solving, or engineering acceptance semantics.

v0.92 advances that shared projection/hash to the signed-zero-stable v3 network-result scope. Uncertainty-corner trace, terminal, and selected-state fingerprints now also cover network/reference identity, inner Newton iteration metadata, mass-balance tolerance, variable-friction convergence/configuration, and chronological outer-iteration history. Terminal projection mismatch paths therefore localize corruption across the expanded deterministic solver-provenance state without changing physical uncertainty or acceptance semantics.

v0.93 propagates supplied-point v3 network-result hash and projection replay across every evaluated nonlinear uncertainty corner, independent of whether the corner ultimately solves an operating point. Aggregate evidence separates incomplete supplied-point coverage from actual hash/projection mismatches and records consistent corners plus hash, projection, and combined violation corner indices/counts. Complete no-intersection cases remain fully auditable while a pre-failure evaluated prefix remains explicit rather than being mistaken for fabricated complete coverage.

v0.94 adds selected operating network-state projection replay to every solved uncertainty corner. Aggregate evidence records projection-replay evidence and consistency counts, exact violating corner indices, structured mismatch details, total mismatch records, and field-local maximum numeric-error witnesses while preserving the separate selected-state SHA-256 and scalar replay evidence. Unresolved corners correctly have no selected operating state; v0.93 supplied-point replay remains the provenance path for their successfully evaluated supplied-point prefix.
