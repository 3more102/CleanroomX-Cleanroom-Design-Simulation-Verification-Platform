...
## Bisection decision-trace provenance
...
v0.74 includes iteration-limit cases in the trace aggregate without reclassifying them as solved. Each affected corner retains its completed L/H-only sequence, reports `bisection_iteration_limit` as the trace termination reason, and must satisfy the same length, sign, midpoint, contiguous-iteration, and state-transition replay checks. The aggregate reports solved and iteration-limit trace counts separately plus exact terminal-outcome violation indices. A non-converged iteration-limit corner still contributes no accepted operating point or complete uncertainty envelope.
...
- operating-point search provenance for solved supplied-point/bisection cases and iteration-limit cases, including exact affected corner indices, remaining signed-bracket geometry/invariants, v0.74 replay-audited L/H trace coverage, and tied worst consistency-error provenance without accepting a non-converged operating point;
