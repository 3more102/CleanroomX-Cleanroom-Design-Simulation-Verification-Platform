...
v0.91 propagates selected operating network-state projection replay across every solved uncertainty corner. Aggregate evidence records projection replay coverage, consistent and violating corner indices, total mismatch records, and exact field paths inside violating selected-state projections while preserving the existing selected-state SHA-256 replay and pressure replay evidence. These diagnostics do not alter corner generation, nonlinear solving, root selection, or engineering acceptance semantics.
