...
## Bisection iteration-limit terminal evidence

v0.68 preserves the bounded root-search state even when an evaluated nominal or uncertainty corner does not converge within `max_operating_iterations`. The solver records the signed terminal bracket after the final allowed contraction, the last evaluated airflow and residual, which bracket endpoint that evaluation became, and binary-contraction invariant evidence. The uncertainty aggregate keeps exact affected corner indices and tied source-corner provenance for the largest terminal bracket width, half-width, normalized width, and raw width-fraction consistency error.

Iteration-limit cases remain unresolved and therefore still prevent a complete uncertainty envelope. The retained terminal geometry is numerical failure-state provenance only; it is not a physical airflow uncertainty interval, interpolation-error bound, statistical confidence interval, stability margin, manufacturer operating region, commissioning criterion, or equipment-acceptance limit.
