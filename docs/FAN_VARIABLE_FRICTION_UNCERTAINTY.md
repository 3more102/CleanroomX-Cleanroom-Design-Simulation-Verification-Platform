...
v0.73 adds exact supplied-point index intervals to solver-eligible discrete candidates and computes the minimum index-step separation between the selected candidate interval and each alternative candidate interval. The uncertainty aggregate preserves the minimum index-space separation and tied source-corner witnesses. This is discrete sample-grid topology only; it does not infer an additional continuous root, interpolation error, physical uncertainty, robustness/stability margin, stall/surge behavior, manufacturer operating region, commissioning/certification result, or equipment-acceptance limit.
