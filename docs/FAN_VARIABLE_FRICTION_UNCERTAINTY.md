...
v0.91 advances those replay hashes to the v3 signed-zero-normalized canonicalization. Every retained floating-point value equal to zero is serialized as `0.0`, preventing `-0.0`/`0.0` representation differences from becoming false uncertainty-corner replay violations without changing any nonzero evidence or acceptance semantics.
