...
v0.75 extends that replay model to non-converged iteration-limit searches. Every budgeted midpoint decision is retained, the expected no-T termination pattern is checked, and the final L/H decision is replayed into the post-budget remaining signed-residual bracket. The aggregate reports exact iteration-limit trace-coverage and violation-corner provenance plus the maximum retained iteration-limit trace length. No root is accepted and no physical limit is inferred.
