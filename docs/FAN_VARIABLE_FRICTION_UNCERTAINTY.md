...
v0.53 carries the exact tied worst-corner sources into each configured solver-tolerance check and identifies the maximum utilization ratio together with every tied critical metric. Standalone reports and engineering dossiers expose this evidence so the numerical convergence bottleneck can be audited without detaching it from the uncertainty inputs that produced it. A study with no utilizable solved-corner tolerance evidence reports no critical metric.
...
- configured solver-tolerance checks for operating-pressure residual, resistance closure, and mass balance, including utilization ratio, remaining numerical margin, exact tied source-corner provenance, maximum utilization, critical metric keys, and complete-vs-partial coverage state; and
