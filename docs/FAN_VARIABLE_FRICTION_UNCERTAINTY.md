...
v0.92 advances that canonical projection to the v3 signed-zero-normalized form across uncertainty corners. All floating values equal to zero are represented as `0.0` before projection comparison and hashing, so `-0.0`/`0.0` representation differences do not become false projection or fingerprint replay violations; nonzero evidence and acceptance semantics are unchanged.
