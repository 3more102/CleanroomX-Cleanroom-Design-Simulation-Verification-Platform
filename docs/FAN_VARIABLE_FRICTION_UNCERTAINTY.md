...
- selected automatic-friction rectangular-duct widths and heights; and
- optional fan, motor, and VFD efficiency bounds used only for power-evidence propagation.
...
`power_efficiency_uncertainty` is an object keyed by `fan_efficiency`, `motor_efficiency`, and/or `vfd_efficiency`. Each entry supplies only `uncertainty_abs` and optional provenance; the corresponding nominal value must already exist in `power_efficiencies`. Bounds must remain inside (0, 1]. Efficiency uncertainty is power-only: it does not alter the fan curve, duct resistances, or hydraulic operating point. `max_power_cases` defaults to 2048 and limits the product of hydraulic corners and unique efficiency endpoint combinations before power cases are materialized.
...
- when efficiency bounds are configured, the exact hydraulic × efficiency power-case count, evaluated power ranges, and tie-aware source records including the efficiency combination for each extremum;
...
- aggregate worst solved-corner operating-pressure residual, resistance-closure error, mass-balance residual, pressure-law residual, and iteration counts with tied source-corner context; and
- provenance completeness for the fan curve and every bounded input.

For indeterminate studies, v0.47 retains unresolved-corner diagnostics but still emits no complete operating-point (including air power) or internal edge-flow envelope. v0.51 solver-quality aggregation may still summarize any solved evaluated corners, but it marks complete-study coverage false and never presents partial numerical evidence as complete verification. Configured pressure, resistance-closure, and mass-balance tolerances are preserved; pressure-law residual and iteration maxima remain diagnostics without an invented acceptance threshold. These min/max values are evaluated-corner ranges only. The v0.46 compact witness index resolves deterministically to the first matching corner for backward-compatible single-witness access, while `operating_point_extrema_sources` preserves every numerically matching tie and the bounded inputs that define those cases. The Markdown report surfaces the same operating-point source evidence, edge-flow first-witness indices, and v0.48 tie-aware internal edge-flow extrema-source records. The v0.49 air-power range is likewise the min/max of evaluated corners only. v0.50 extends that same evaluated-corner treatment to the solver's fan-side power chain. Shaft power requires an explicit fan efficiency; electrical input and specific fan power require explicit fan, motor, and VFD efficiencies. Missing efficiencies remain unavailable rather than being guessed. In v0.50 those efficiencies are fixed deterministic inputs; v0.52 can promote selected supplied efficiencies to explicit bounded power-only uncertainty dimensions without changing the hydraulic corner solutions. v0.51 solver-quality aggregation remains independent and may summarize solved hydraulic corners even when a complete operating-point envelope is withheld. These records are not sensitivity coefficients and are not claimed to be mathematically guaranteed continuous-interval extrema for every interior combination.

## Input and CLI

See `examples/fan_variable_friction_uncertainty_demo.json` for the original fixed-pressure/K workflow, `examples/fan_variable_friction_physical_uncertainty_demo.json` for v0.39 roughness/viscosity/density bounds, and `examples/fan_variable_friction_geometry_uncertainty_demo.json` for v0.40 duct-length/circular-diameter bounds. `examples/fan_variable_friction_rectangular_uncertainty_demo.json` demonstrates v0.41 rectangular width/height bounds. `examples/fan_variable_friction_fan_curve_uncertainty_demo.json` demonstrates v0.42 selected fan-point pressure bounds. `examples/fan_variable_friction_fan_airflow_uncertainty_demo.json` demonstrates v0.43 selected fan-point airflow-coordinate bounds. `examples/fan_variable_friction_speed_uncertainty_demo.json` demonstrates v0.44 bounded fan-speed ratio uncertainty. `examples/fan_variable_friction_curve_scenarios_demo.json` demonstrates v0.45 correlated whole fan-curve scenarios. `examples/fan_variable_friction_efficiency_uncertainty_demo.json` demonstrates v0.52 bounded fan/motor/VFD efficiency uncertainty.
...
`max_corner_cases` defaults to 256 for hydraulic uncertainty. Every non-zero hydraulic bounded dimension doubles the two-endpoint corner space, while zero-width dimensions are deduplicated. `max_power_cases` defaults to 2048 and separately limits hydraulic-corner × efficiency-endpoint power evidence. CleanroomX rejects an analysis that exceeds either configured limit; it never silently truncates a case set.
