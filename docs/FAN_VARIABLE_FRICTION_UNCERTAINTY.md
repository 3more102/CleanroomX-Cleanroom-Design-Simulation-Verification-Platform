...
## Bisection iteration-limit evidence

v0.70 preserves bounded-search state when an evaluated nonlinear case exhausts
`max_operating_iterations` before satisfying the configured operating-pressure
tolerance. The failed case remains non-converged and exposes no operating point.
Its search evidence retains the final evaluated midpoint and residual plus the
remaining strict signed-residual bracket after that budgeted evaluation.

The remaining bracket records low/high airflow, endpoint residuals, width,
half-width, width relative to the original supplied interpolation segment,
completed binary contraction steps, the iteration-implied width fraction, the
actual width fraction, and their raw floating-point consistency error. Across
uncertainty corners, CleanroomX reports exact iteration-limit corner indices,
remaining-bracket invariant coverage, strict sign-bracket violations, and tied
source-corner provenance for the largest raw consistency error.

This is implementation/search-state evidence only. It is not physical airflow
uncertainty, an interpolation-error bound, statistical confidence, a continuous
worst-case guarantee, dynamic-stability evidence, or an equipment-acceptance
criterion.
