...
v0.85 adds the same exact mismatch and tied worst-witness provenance for each terminal bisection bracket. Both solved final brackets and post-decision iteration-limit remaining brackets are independently re-solved at their low/high endpoints, and uncertainty aggregation reports exact terminal replay violation corners/details plus the maximum terminal pressure-component replay error.
