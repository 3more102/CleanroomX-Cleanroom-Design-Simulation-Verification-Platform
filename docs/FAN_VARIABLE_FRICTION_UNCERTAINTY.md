...
v0.89 changes the network-state fingerprint canonicalization used by those replay checks to an order-invariant v2 form for named node, edge, and variable-friction closure collections. Uncertainty aggregation semantics do not change; equivalent solved states with different enumeration order no longer produce false replay violations.
