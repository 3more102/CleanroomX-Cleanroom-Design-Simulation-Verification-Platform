...
## Terminal root-search interval evidence

v0.65 retains the operating-point solver's actual termination search geometry
for every solved uncertainty corner. Bounded-bisection cases preserve the
initial supplied-point sign-change bracket and the final sign-consistent
bracket after the accepted midpoint evaluation is incorporated by residual
sign. The evidence includes the initial and terminal airflow spans, terminal
endpoint residuals, and the terminal-to-initial span contraction ratio.

A solved case that terminates because an already supplied fan point satisfies
the configured pressure-residual tolerance is recorded separately as a
`supplied_point_tolerance_contact`. Such a case has no bisection span or
contraction ratio; CleanroomX does not fabricate one.

The aggregate result reports complete-versus-partial search-evidence coverage,
counts and exact indices for bisection versus supplied-point-contact corners,
and tied source-corner provenance for the largest terminal bisection span and
largest contraction ratio. These quantities are numerical root-search
provenance only. They are not physical airflow uncertainty, interpolation-error
bounds, dynamic-stability or stall/surge margins, manufacturer operating
regions, commissioning/certification criteria, or equipment-acceptance limits.
