...
v0.79 independently recomputes strict sign bracketing and arithmetic midpoint geometry from the retained numeric trace state rather than trusting stored booleans. v0.80 adds an independent residual-component layer: every trace step retains fan and system pressures at its low/high endpoints and midpoint, and each recorded fan-minus-system residual is checked against the direct pressure difference. The aggregate reports exact pressure-component violation corners and tied witnesses for the largest absolute residual-component error. These are implementation-provenance checks only; they do not quantify physical uncertainty, interpolation error, continuous root guarantees, stability/stall/surge behavior, manufacturer operating limits, commissioning/certification status, or equipment acceptance.
