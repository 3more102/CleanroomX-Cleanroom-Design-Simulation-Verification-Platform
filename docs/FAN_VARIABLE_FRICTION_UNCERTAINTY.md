...
v0.90 propagates terminal network-state projection replay across solved and iteration-limit uncertainty corners. The aggregate records projection-consistent and violating corner indices, exact low/high endpoint positions, and deterministic field mismatch paths while preserving the separate SHA-256 fingerprint replay evidence. These are implementation-provenance diagnostics only; they do not modify corner selection, nonlinear solving, or engineering acceptance semantics.

v0.91 propagates selected operating network-state projection replay across every solved uncertainty corner. Aggregate evidence records applicability, exact replay coverage, consistent/inconsistent counts, violating corner indices, total mismatch records, exact field paths, detailed recorded/recomputed/type mismatch evidence, and tied per-field maximum numerical-error witnesses while preserving the existing selected-state SHA-256 replay and pressure replay evidence. Incomplete checking is reported as incomplete coverage rather than being treated as complete. These diagnostics do not alter corner generation, nonlinear solving, root selection, or engineering acceptance semantics, and they are not physical uncertainty quantification, cleanroom certification, CFD validation, fan acceptance, commissioning evidence, global-root-uniqueness proof, or stall/surge analysis.

v0.92 propagates full low/midpoint/high bisection network-state projection replay across every applicable solved or iteration-limit trace corner. Aggregation distinguishes applicable corners, any-evidence coverage, complete state-position coverage, consistent corners, inconsistent corners, and incomplete-coverage corners; it retains exact corner/iteration/position/path mismatches, detailed retained/recomputed/type evidence, explicit coverage gaps, total mismatch counts, and tied per-field maximum numerical-error witnesses. A missing replay check is therefore never promoted to complete consistency. Unsolved/no-intersection/non-converged engineering outcomes retain their original semantics. This remains numerical/provenance verification only and is not cleanroom certification, CFD validation, fan acceptance, commissioning evidence, proof of global root uniqueness or physical stability, stall/surge analysis, manufacturer operating-envelope validation, physical uncertainty quantification, or statistical confidence analysis.

v0.93 aggregates supplied-point internal network-state replay across every evaluated uncertainty corner, including corners that ultimately have no fan/system intersection or become non-converged after only a successfully evaluated supplied-point prefix. Aggregation reports evidence and complete-coverage counts, consistent/inconsistent/incomplete corners, separate hash and projection violation corner indices, total point-level projection mismatch records, exact corner/point/path evidence, explicit coverage gaps, and tied per-field maximum numerical-error witnesses. Missing supplied points or failed independent replays remain coverage gaps rather than being counted as clean consistency, and replay diagnostics do not alter any corner's engineering status.

v0.94 advances every retained network-state identity/projection used by uncertainty-corner replay to the same signed-zero-stable solver-provenance-complete v3 canonicalization. Named node/edge/closure enumeration changes no longer alter canonical identity, while chronological outer-iteration history remains order-sensitive. Existing corner mismatch aggregation therefore also covers solver metadata/configuration/history fields without changing corner enumeration, nonlinear solving, uncertainty semantics, or engineering acceptance.
