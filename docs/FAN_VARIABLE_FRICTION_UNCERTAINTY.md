...
v0.73 applies the same trace model to `bisection_iteration_limit` outcomes. These unresolved cases retain every completed L/H midpoint decision, require the trace length to match the recorded operating-iteration count, require no terminal `T`, and remain explicitly non-converged with no accepted operating point. The aggregate distinguishes solved and iteration-limit trace coverage and reports exact corner indices for trace-length, sign-bracket, midpoint-geometry, or terminal-outcome inconsistencies.
