...
v0.91 adds supplied-point network-state replay coverage to every evaluated nonlinear uncertainty corner, independent of whether that corner ultimately solves an operating point. The summary now records replay-evidence coverage, complete supplied-point coverage, incomplete-coverage corner indices, consistent corners, violating corners, and aggregate violation counts. This keeps complete no-intersection cases auditable and makes partial pre-failure coverage explicit without converting missing evaluations into fabricated hash mismatches.
