...
a missing value is shown as unavailable. Relationship arrows are drawn only from the active
analysis's explicit `pressure_cascade` records. When both linked rooms have supplied pressure,
the arrow reports the observed delta against the configured minimum and renders pass/fail state;
otherwise it renders an explicit unavailable state. CleanroomX does not invent missing pressure,
airflow values, or a calculated pressure field.
...
The smoke command fails closed unless the packaged demo loads, the real 2D room geometry renders,
the 3D room extrusion renders, the active analysis runs, and pressure-cascade relationship
