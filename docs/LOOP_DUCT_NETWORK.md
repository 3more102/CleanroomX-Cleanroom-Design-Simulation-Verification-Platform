...
CleanroomX v0.24 bridges the v0.21 duct/friction calculations and the v0.23 fixed-resistance loop solver. It derives each loop edge's fixed quadratic resistance from explicit duct geometry and loss inputs, then solves the resulting connected pressure network.
