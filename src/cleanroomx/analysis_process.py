...
def _send_terminal_message(connection: Connection, message: WorkerMessage) -> None:
    try:
        ...
        # The parent may have cancelled or closed while the child was finishing.
        pass
...
def _analysis_process_main(
    ...
    """Execute one application analysis in an isolated child process."""
    try:
        run = run_analysis(
            kind,
            payload,
...
class AnalysisProcess:
    """One-at-a-time process boundary for interactive desktop analyses.
    ...
    def start(
        ...
        *,
        ...
        # The spawned child owns its copy of the sending end after start().
        send_connection.close()
    ...
    def poll(self) -> WorkerMessage | None:
        ...
        # Once the process is dead, consume a complete terminal pipe message.
        if self._message is None and connection.poll():
            try:
                self._message = connection.recv()
            except EOFError:
    ...
    def shutdown(self) -> None:
        """Boundedly stop and reap an active child before releasing ownership."""
        process = self._process
        if process is None:
            return
        ...
        if process.is_alive():
            ...
            process.join(timeout=self._cancel_kill_grace_seconds)
        if process.is_alive():
            raise RuntimeError("analysis worker could not be stopped")
