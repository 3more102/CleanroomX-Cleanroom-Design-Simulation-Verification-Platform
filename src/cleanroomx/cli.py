...
from .verification import ReportStatus, verify_room
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verify_project_parser = sub.add_parser(
        "verify-project",
        help=(
            "Verify a multi-room project including configured pressure-cascade "
            "requirements"
        ),
    )
...
def _verification_exit_code(status: ReportStatus) -> int:
    if status == "fail":
        return 2
...
def main() -> int:
    ...
    if args.command == "verify":
        ...
        return _verification_exit_code(report.status)
