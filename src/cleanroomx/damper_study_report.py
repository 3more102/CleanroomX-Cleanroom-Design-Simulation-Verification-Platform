...
def markdown_loop_damper_study_report(result: dict) -> str:
    ...
    lines = [
        f"# CleanroomX Loop Damper-Resistance Study — {markdown_text(result['study'])}",
        "",
        f"- Base network: **{markdown_text(result['base_network'])}**",
    ...
    for edge in baseline["edges"]:
        lines.append(
            f"| {markdown_text(edge['name'])} | {edge['airflow_m3_h']} | "
    ...
    for case in result["cases"]:
        ...
        lines.extend(
            [
                "",
                f"## Case — {markdown_text(case['name'])}",
                "",
        ...
        for item in case["adjustments"]:
            lines.append(
                f"| {markdown_text(item['edge'])} | {item['resistance_multiplier']} | "
        ...
        for row in case["flow_changes"]:
            ...
            lines.append(
                f"| {markdown_text(row['edge'])} | {row['baseline_airflow_m3_h']} | "
