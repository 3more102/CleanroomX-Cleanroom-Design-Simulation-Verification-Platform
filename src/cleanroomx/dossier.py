...
def _hvac_summary(result: dict | None) -> dict:
    if result is None:
        return {
            ...
            "room_count": 0,
            "fan_curve_duty_status": "not_included",
        }
    ...
    fan_duty = result.get("fan_curve_duty_check")
    fan_duty_status = (
        str(fan_duty.get("status", "unknown"))
        if fan_duty is not None
        else "not_included"
    )

    if failed or fan_duty_status == "fail":
        status = "fail"
    elif fan_duty_status == "outside_supplied_range":
        status = "outside_supplied_range"
    elif fan_duty_status not in {"not_included", "pass"}:
    ...
    return {
        ...
        "fan_curve_duty_status": fan_duty_status,
    }
...
def summarize_dossier_components(
    ...
    adverse = {
        ...
        "hvac_fan_duty_failures": int(
            components["hvac"].get("fan_curve_duty_status") == "fail"
        ),
        "hvac_fan_duty_outside_supplied_range": int(
            components["hvac"].get("fan_curve_duty_status")
            == "outside_supplied_range"
        ),
        "hvac_fan_duty_unknown": int(
            components["hvac"].get("fan_curve_duty_status")
            not in {"not_included", "pass", "fail", "outside_supplied_range"}
        ),
