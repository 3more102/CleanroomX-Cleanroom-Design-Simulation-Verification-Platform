    ...
    if result.get("fan_variable_friction_uncertainty_analyses"):
        ...
        for item in result[
            ...
            if power_uncertainty_ranges is not None:
                power_case_availability = item.get(
                ...
                electrical_cov = power_case_availability.get(
                    "electrical_input_kw", {}
                )
                sfp_cov = power_case_availability.get(
                    "specific_fan_power_w_per_m3_s", {}
                )
                electrical_text = (
                    "—"
                ...
                lines.append(
                    ...
                    f"electrical input {electrical_text} "
                    f"[{electrical_cov.get('status', '—')}]; "
                    f"SFP {sfp_text} [{sfp_cov.get('status', '—')}]."
                )
