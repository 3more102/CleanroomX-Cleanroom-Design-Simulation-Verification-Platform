...
def solve_parallel_branch_flows(network: ParallelFlowNetwork) -> dict:
    ...
    for path, resistance in zip(network.paths, resistances):
        ...
        section_pressure_drop_sum_pa = sum(
            section.resistance_pa_per_m3_s_squared * airflow_m3_s**2
        ...
        path_results.append(
            {
                ...
                "section_pressure_drop_sum_pa": round(section_pressure_drop_sum_pa, 4),
                "pressure_balance_error_pa": round(
                    section_pressure_drop_sum_pa - common_pressure_drop_pa, 9
                ),
                "sections": sections,
            }
        )
    ...
    return {
        ...
        "max_abs_pressure_balance_error_pa": round(
            max(abs(path["pressure_balance_error_pa"]) for path in path_results),
            9,
        ),
        "scope_note": (
