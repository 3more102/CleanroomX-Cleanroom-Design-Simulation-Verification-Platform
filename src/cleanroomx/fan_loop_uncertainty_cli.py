...
from .fan_loop_uncertainty import (
    analyze_fan_loop_network_uncertainty,
)
from .fan_loop_uncertainty_io import (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Deterministic bounded uncertainty for a fan-driven "
            "fixed-resistance loop network"
        ),
    )
    parser.add_argument(
        "study",
        help="Path to fan/loop uncertainty JSON",
    )
    parser.add_argument(
        "--format",
