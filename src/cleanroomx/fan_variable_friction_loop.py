...
def _bisection_decision_trace_audit(
    ...
    *,
    ...
    for step in trace:
        ...
        absolute_midpoint_residual_identity_error = abs(
            recorded_midpoint_residual - recomputed_midpoint_residual
        )
        midpoint_residual_matches_pressure_components = math.isclose(
        ...
        raw_state_checks.append(
            {
                ...
                "absolute_midpoint_residual_identity_error_pa": (
                    absolute_midpoint_residual_identity_error
                ),
        ...
        decision_semantic_checks.append(
            {
                ...
                "decision_matches_pressure_component_semantics": (
                    recorded_decision == expected_component_decision
                ),
            }
        )
    ...
    return {
        ...
        "maximum_absolute_midpoint_residual_identity_error_pa": max(
            (
                check["absolute_midpoint_residual_identity_error_pa"]
                for check in raw_state_checks
            ),
            default=0.0,
        ),
        ...
        "all_decisions_match_pressure_component_semantics": all(
        ...
        "pressure_component_decision_semantic_violation_iterations": [
            check["iteration"]
            for check in decision_semantic_checks
            if not check["decision_matches_pressure_component_semantics"]
        ],
        ...
        "scope_note": (
            ...
            "midpoint facts from the recorded numeric state, checks the "
            "stored flags against those recomputed facts, and independently "
            "verifies each recorded midpoint residual equals recorded fan "
            "pressure minus recorded system pressure. The decision-semantics "
            "audit "
