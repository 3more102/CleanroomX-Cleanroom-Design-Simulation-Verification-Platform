...
def _operating_point_search_resolution_summary(
    ...
    trace_pressure_component_violation_corner_indices = [
        corner_index
        for corner_index, _corner, _evidence, audit in trace_cases
        if not audit["all_trace_residuals_match_pressure_components"]
    ]
    ...
    return {
        ...
        "bisection_trace_pressure_component_consistent_corner_count": (
            len(trace_cases)
            - len(trace_pressure_component_violation_corner_indices)
        ),
