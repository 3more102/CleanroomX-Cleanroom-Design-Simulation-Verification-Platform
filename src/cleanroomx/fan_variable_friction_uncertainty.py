...
class FanVariableFrictionLoopUncertaintyStudy:
    ...
    def __post_init__(self) -> None:
        ...
        for key, item in self.power_efficiency_uncertainty.items():
            ...
            if not math.isclose(item.value, nominal, rel_tol=1e-12, abs_tol=1e-12):
                raise ValueError(
...
def _power_uncertainty_evidence(
    ...
    nominal_efficiencies = (
        {} if study.power_efficiencies is None else study.power_efficiencies.to_dict()
    )
    cases: list[dict] = []
    for corner_index, corner in enumerate(corners):
        ...
        for values in combinations:
            ...
            power = analyze_fan_pressure_power(
                point["airflow_m3_h"], point["fan_pressure_pa"], efficiencies
            )
            cases.append(
                {
                    "corner_index": corner_index,
    ...
    for key, unit in metric_specs:
        values = [
            ...
            if case["power_evidence"].get(key) is not None
        ]
        if not values:
            ranges[key] = None
            sources[key] = None
        ...
        def _matching(target: float) -> list[dict]:
            ...
            for case_index, value in values:
                if not math.isclose(value, target, rel_tol=1e-12, abs_tol=1e-9):
                    continue
                case = cases[case_index]
                source = _critical_case_summary(
                    case["corner_index"], corners[case["corner_index"]]
                )
...
def analyze_fan_variable_friction_loop_uncertainty(
    ...
    return {
        ...
        "power_uncertainty_case_count": (
            len(power_uncertainty_cases) if power_uncertainty_cases is not None else 0
        ),
        "power_uncertainty_cases": power_uncertainty_cases,
        "power_uncertainty_ranges": power_uncertainty_ranges,
        "power_uncertainty_extrema_sources": power_uncertainty_extrema_sources,
        "power_evidence_excursions_from_nominal": (
            power_evidence_excursions_from_nominal
        ),
        "traceability": {
        ...
        "engineering_note": (
            ...
            "duct length, circular diameter, rectangular width/height, and "
            "explicit fan/motor/VFD efficiency bounds for power-only propagation. "
            ...
            "explicit efficiency bounds, when supplied, are propagated only "
            "through the power chain after each solved aerodynamic corner and "
            "do not alter airflow or pressure. No missing efficiency value or "
