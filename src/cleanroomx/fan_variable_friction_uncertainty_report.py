...
def markdown_fan_variable_friction_loop_uncertainty_report(
    ...
    power_uncertainty_sources = result.get("power_uncertainty_extrema_sources")
    if power_uncertainty_ranges is not None:
        metric_labels = (
            ("fluid_air_power_kw", "Fluid air power", "kW"),
            ("shaft_power_kw", "Shaft power", "kW"),
            ("electrical_input_kw", "Electrical input", "kW"),
            ("specific_fan_power_w_per_m3_s", "Specific fan power", "W/(m³/s)"),
        )
        lines.extend(
            [
                "",
        ...
        for key, label, display_unit in metric_labels:
            ...
            if evidence is None:
                ...
                lines.append(
                    f"| {label} | {evidence['lower']} | {evidence['upper']} | "
                    f"{display_unit} |"
                )
        ...
        lines.extend(
            [
                ...
                "power evidence only and do not alter the hydraulic operating point. "
                "The reported power ranges cover evaluated hydraulic × efficiency "
                "corner combinations; no missing efficiency or interior aerodynamic "
                "extremum is inferred.",
            ]
        )
