...
class CleanroomXApp:
    ...
    def load_project_path(self, path: str | Path) -> None:
        ...
        if migration_info.migrated:
            # The in-memory model is current schema, but the source file is not.
            # Keep the conversion explicitly unsaved so an ordinary Save cannot
            # silently replace the only legacy copy.
