...
class CleanroomXApp:
    ...
    def run_current(self) -> None:
        ...
        try:
            self._analysis_task = AnalysisTask.start(
                kind,
                payload,
                base_dir=base_dir,
            )
            ...
            self.status_var.set("Analysis could not start")
            messagebox.showerror(
    ...
    def cancel_run(self) -> None:
        ...
        if not self._running or task is None or task.cancel_requested:
            return
        if task.cancel():
    ...
    def _poll_worker(self) -> None:
        ...
        if task is not None:
            try:
                ...
                self._analysis_task = None
                self._set_running(False)
                self.status_var.set("Analysis worker failed")
                messagebox.showerror(
                ...
                if outcome is not None:
                    analysis_id = getattr(self, "_running_analysis_id", None)
                    self._analysis_task = None
                    self._set_running(False)
                    if outcome.status == "cancelled":
                        ...
                        detail = outcome.error or "Analysis worker failed"
                        ...
                        if run is None or analysis_id is None:
                            self.status_var.set("Analysis worker returned invalid state")
                            messagebox.showerror(
                                "Analysis failed",
                                "Analysis worker returned no result ownership metadata.",
                                parent=self.root,
                            )
                        else:
    ...
    def _on_close(self) -> None:
        if self._running and not messagebox.askyesno(
            "Analysis running",
            "A backend analysis is still running. Cancel it and close CleanroomX?",
            parent=self.root,
        ):
            return
        if not self._confirm_project_replacement():
            return
        task = getattr(self, "_analysis_task", None)
        if task is not None:
            task.shutdown()
            self._analysis_task = None
        self._discard_current_autosave()
