    ...
    if args.smoke:
        try:
            ...
            if args.demo:
                ...
                if not app.spatial_workspace.canvas_2d.find_withtag(
                    "pressure_relationship"
                ):
                    raise RuntimeError("2D pressure cascade did not render")
                if not app.spatial_workspace.canvas_3d.find_withtag(
                    "pressure_relationship_3d"
                ):
                    raise RuntimeError("3D pressure cascade did not render")
        except Exception as exc:
            root.destroy()
            print(f"CleanroomX GUI smoke: FAIL — {exc}")
            return 2
