...
def analyze_hvac_project(project: HVACProject) -> dict:
    ...
    duct_network = (
        analyze_duct_network(project.supply_duct_network)
        if project.supply_duct_network is not None
        else None
    )
    ...
    if project.fan_system is not None:
        ...
        duct_override = (
            duct_network["critical_path_pressure_loss_pa"]
    ...
    return {
        ...
        "engineering_note": (
            ...
            "to a fixed ACH, pressure offset, airflow surplus, duct geometry, roughness, "
            "fitting loss, filter pressure drop, or fan duty."
        ),
    }
