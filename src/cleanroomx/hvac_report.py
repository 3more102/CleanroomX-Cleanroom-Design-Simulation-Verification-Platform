    ...
    if result["branch_flow_network"] is not None:
        ...
        lines.extend(
            [
                ...
                f"- Terminal demand source: **{network['terminal_airflow_source']}**.",
                f"- Critical terminal: **{network['critical_terminal']}** "
                f"(room **{network['critical_room']}**).",
                f"- Critical-path pressure drop: **{network['critical_path_pressure_drop_pa']} Pa**.",
                f"- Maximum continuity residual: **{network['max_abs_continuity_residual_m3_h']} m³/h**.",
                "",
        ...
        lines.extend(
            [
                "",
                "| Terminal | Room | Governing airflow m³/h | Source-to-terminal pressure drop Pa |",
                "|---|---|---:|---:|",
            ]
        )
        for terminal in network["terminals"]:
            lines.append(
                f"| {terminal['node']} | {terminal['room_name']} | "
                f"{terminal['airflow_m3_h']} | "
