...
def _verify_file_revision(\n    path: Path,\n    *,\n    expected_size: int,\n    expected_sha256: str,\n    stage: str,\n    committed: bool,\n) -> None:\n    """Verify one stable file against an already-captured byte identity."""\n    try:\n        stat_result, actual_sha256 = _stable_file_sha256(path)\n    except OSError as exc:\n        raise AtomicWriteVerificationError(\n            path,\n            stage=stage,\n            expected_size=expected_size,\n            expected_sha256=expected_sha256,\n            actual_size=None,\n            actual_sha256=None,\n            committed=committed,\n        ) from exc\n    if stat_result.st_size != expected_size or actual_sha256 != expected_sha256:\n        raise AtomicWriteVerificationError(\n            path,\n            stage=stage,\n            expected_size=expected_size,\n            expected_sha256=expected_sha256,\n            actual_size=stat_result.st_size,\n            actual_sha256=actual_sha256,\n            committed=committed,\n        )\n\n\ndef atomic_write_stream(\n    path: str | Path,\n    writer: StreamWriter,\n    *,\n    before_replace: BeforeReplace | None = None,\n) -> Path:\n    """Atomically publish incrementally generated bytes through shared durability rules."""\n    if not callable(writer):\n        raise TypeError("writer must be callable")\n\n    destination = Path(path)\n    destination.parent.mkdir(parents=True, exist_ok=True)\n\n    existing_mode: int | None = None\n    try:\n        existing_mode = stat.S_IMODE(destination.stat().st_mode)\n    except FileNotFoundError:\n        pass\n\n    temp_path: Path | None = None\n    try:\n        with tempfile.NamedTemporaryFile(\n            mode="w+b",\n            prefix=f".{destination.name}.",\n            suffix=".tmp",\n            dir=destination.parent,\n            delete=False,\n        ) as handle:\n            temp_path = Path(handle.name)\n            writer(handle)\n            if existing_mode is not None and hasattr(os, "fchmod"):\n                os.fchmod(handle.fileno(), existing_mode)\n            handle.flush()\n            os.fsync(handle.fileno())\n\n        staged_stat, expected_sha256 = _stable_file_sha256(temp_path)\n        expected_size = staged_stat.st_size\n        if before_replace is not None:\n            before_replace()\n        temp_path.replace(destination)\n        temp_path = None\n        try:\n            _fsync_directory(destination.parent)\n        except OSError as exc:\n            raise AtomicWriteDurabilityError(destination, exc) from exc\n        _verify_file_revision(\n            destination,\n            expected_size=expected_size,\n            expected_sha256=expected_sha256,\n            stage="committed write",\n            committed=True,\n        )\n    except BaseException:\n        if temp_path is not None:\n            temp_path.unlink(missing_ok=True)\n        raise\n    return destination\n\n\ndef atomic_write_text(
    path: str | Path,
    text: str,
    *,
    before_replace: BeforeReplace | None = None,
