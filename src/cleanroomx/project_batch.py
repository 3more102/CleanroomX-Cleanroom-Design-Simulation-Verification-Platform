...
def render_project_batch_markdown(batch: ProjectBatchRun) -> str:
    """Render a concise summary without allowing project-controlled Markdown structure."""
    lines = [
    ...
    if not batch.source_stable_during_run:
        boundary = markdown_text(batch.source_change_stage or "unknown")
        if batch.source_change_analysis_id:
            boundary += " at analysis " + markdown_text(batch.source_change_analysis_id)
        lines.append(f"- Source-change boundary: {boundary}")
        if batch.source_check_error:
            lines.append(
                f"- Source-check error: {markdown_text(batch.source_check_error)}"
            )
    ...
    for outcome in batch.outcomes:
        lines.append(
            "### "
            + markdown_text(outcome.analysis_name)
            + " ("
            + markdown_text(outcome.analysis_id)
            + ", "
            + markdown_text(outcome.kind)
            + ")"
        )
        lines.append("")
        if outcome.execution_state == "completed":
            ...
            lines.append(
                "- Analysis status: "
                + markdown_text(outcome.run.get("status", "complete"))
            )
            ...
            lines.append(
                "- Error: "
                + markdown_text(outcome.error_type or "Error")
                + ": "
                + markdown_text(outcome.error_message or "")
            )
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
