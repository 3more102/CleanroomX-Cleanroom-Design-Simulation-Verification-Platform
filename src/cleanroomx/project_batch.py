...
def render_project_batch_markdown(batch: ProjectBatchRun) -> str:
    lines = [
        f"# CleanroomX project batch — {batch.project_name}",
        "",
    ...
    if not batch.source_stable_during_run:
        lines.append(
            ...
            + (
                f" at analysis `{batch.source_change_analysis_id}`"
                if batch.source_change_analysis_id
                else ""
            )
        )
        if batch.source_check_error:
            lines.append(f"- Source-check error: {batch.source_check_error}")
    ...
    for outcome in batch.outcomes:
        lines.append(
            f"### {outcome.analysis_name} (`{outcome.analysis_id}`, "
            f"`{outcome.kind}`)"
        )
        lines.append("")
        if outcome.execution_state == "completed":
            ...
            lines.append(
                f"- Error: {outcome.error_type or 'Error'}: "
