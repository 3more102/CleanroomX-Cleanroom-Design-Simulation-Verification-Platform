...
def markdown_qualification_report(result: dict) -> str:
    lines = [
        f"# CleanroomX Qualification Uncertainty Report — {result['analysis']}",
        "",
    ...
    if result["measurements"]:
        ...
        for item in result["measurements"]:
            ...
            lines.append(
                f"| {item['name']} | {_fmt(item['interval']['lower'])} to "
                f"{_fmt(item['interval']['upper'])} {item['unit']} | "
                f"{symbol} {_fmt(req['limit'])} {req['unit']} | "
                f"{item['status'].upper()} | {req['reference'] or '—'} |"
            )
        lines.append("")
    ...
    if result["pressure_cascades"]:
        ...
        for item in result["pressure_cascades"]:
            req = item["requirement"]
            lines.append(
                f"| {item['name']} | {_fmt(item['nominal_delta_pa'])} Pa | "
    ...
    if traceability["missing_provenance"]:
        lines.append("- Missing provenance: " + ", ".join(traceability["missing_provenance"]))
