...
class RecoverySample:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(uncertainty) or uncertainty < 0:
            raise ValueError("concentration_uncertainty_abs must be finite and >= 0")
    ...
    @property
    def concentration_lower_per_m3(self) -> float:
        return max(0.0, self.concentration_per_m3 - self.concentration_uncertainty_abs)

    @property
    def concentration_upper_per_m3(self) -> float:
...
class RecoveryTestSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if self.uncertainty_reference is not None and not self.uncertainty_reference.strip():
            raise ValueError("uncertainty_reference cannot be empty when provided")
