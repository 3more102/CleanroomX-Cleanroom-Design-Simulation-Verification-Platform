...
from .project import ProjectDocument, atomic_write_text, project_from_dict, save_project_document
...
def save_project_document_with_revision(
    ...
    *,
    ...
    """Save a project while preserving the exact previous valid on-disk version.

    Existing destination bytes are validated and archived before replacement. If
    archival fails, the explicit save is aborted so CleanroomX never destroys a
    previous project version without first preserving it.
    """
    ...
    return save_project_document(destination, project)
