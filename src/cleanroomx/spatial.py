...
def derive_layout_from_analysis(analysis: Any) -> dict:
    ...
    for index, raw in enumerate(raw_rooms):
        ...
        room = {
            ...
            "engineering_ref": create_engineering_ref(analysis, raw, room_name=name),
            "x_m": x_cursor,
            "y_m": 0.0,
            "length_m": length,
            "width_m": width,
    ...
    if getattr(analysis, "kind", "") == "room_verification":
        ...
        if "observed_pressure_pa" in analysis.input and "pressure_pa" in source:
            if analysis.input.get("observed_pressure_pa") != source["pressure_pa"]:
                ...
                changed = True
        baseline_changed = refresh_sync_baselines(layout, analysis)
        return changed or baseline_changed
...
def sync_analysis_to_layout(layout: dict, analysis: Any) -> bool:
    ...
    if analysis is None or not isinstance(getattr(analysis, "input", None), dict):
        return False
    return _synchronize_analysis_to_layout(layout, analysis)
...
def validate_layout(value: Any, analysis: Any = None) -> list[dict]:
    """Return deterministic spatial diagnostics without hiding invalid edit state."""
    issues: list[dict] = []
    ...
    if isinstance(raw_rooms, list):
        for index, raw in enumerate(raw_rooms):
            ...
            if (
                "floor_elevation_m" in raw
                and _optional_finite(raw.get("floor_elevation_m")) is None
            ):
                issues.append(
                    {
...
class SpatialDesignWorkspace(ttk.Frame):
    ...
    def _pressure_relationships(self) -> list[tuple[dict, dict, dict]]:
        ...
        for item in self._overlay()["relationships"]:
            high = room_by_id.get(str(item.get("higher_room_id") or ""))
            low = room_by_id.get(str(item.get("lower_room_id") or ""))
            if high is None or low is None:
                continue
            relationships.append((high, low, item))
    ...
    def _draw_2d(self) -> None:
        ...
        for room in self.layout["rooms"]:
            ...
            hovered = self._hover == _Hit("room", room["id"])
            outline = (
                "#1d4ed8"
                if selected
                else (
            ...
            pressure = pressure_by_id.get(room["id"], {})
            pressure_value = pressure.get("pressure_pa")
            fill = (
                _pressure_fill(pressure_value, pmin, pmax)
        ...
        if self._show_devices.get():
            ...
            for device in self.layout["devices"]:
                ...
                hovered = self._hover == _Hit("device", device["id"])
                device_outline = (
                    "#c0392b"
                    if selected
                    else (
