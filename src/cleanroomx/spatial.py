...
def sync_analysis_to_layout(layout: dict, analysis: Any) -> bool:
    """Pull mapped engineering geometry into the spatial layout explicitly.

    The complete mapping is validated before any spatial room is changed. X/Y
    placement and unrelated spatial metadata are preserved.
    """
    if analysis is None or not isinstance(getattr(analysis, "input", None), dict):
        return False
...
def pressure_overlay_state(
    ...
    """Describe room-pressure rendering from explicit input/result evidence only."""
    normalized = normalize_layout(layout)
    ...
    if kind == "room_verification":
        ...
        targets = [room for room in payload["rooms"] if isinstance(room, dict)]
    else:
        targets = []
    target_by_name = {
    ...
    for index, room in enumerate(normalized["rooms"]):
        ...
        target = None
        if kind == "room_verification" and index == 0 and targets:
            target = targets[0]
        ...
        evidence.append(
            {
                ...
                "pressure_target_pa": target_value,
                "source": source,
                "status": status,
