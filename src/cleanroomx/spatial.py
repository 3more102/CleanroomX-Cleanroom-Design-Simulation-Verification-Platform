...
class SpatialDesignWorkspace(ttk.Frame):
    ...
    def fit_views(self) -> None:
        ...
        min_z = min(
            (room.get("floor_elevation_m", floor_z) for room in self.layout["rooms"]),
            default=floor_z,
        )
        max_z = max(
            (
                room.get("floor_elevation_m", floor_z) + room["height_m"]
                for room in self.layout["rooms"]
            ),
            default=floor_z + 3.0,
        )
        model_span = max(1.0, max_x - min_x, max_y - min_y, max_z - min_z)
    ...
    def _pressure_relationships(self) -> list[dict]:
        if not self._show_relationships.get():
            return []
        return pressure_relationship_records(self.layout, self._analysis_getter())
    ...
    def _draw_relationships_2d(self) -> None:
        ...
        for relationship in self._pressure_relationships():
            ...
            dash = (6, 3) if state == "unavailable" else ()
