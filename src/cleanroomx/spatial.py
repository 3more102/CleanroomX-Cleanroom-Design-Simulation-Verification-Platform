    ...
    return (
        tuple(
            (
                ...
                room.get("width_m"),
                room.get("height_m"),
                room.get("floor_elevation_m"),
                room.get("analysis_room_name"),
                room.get("analysis_id"),
            )
            for room in rooms
...
def validate_layout(value: Any, analysis: Any = None) -> list[dict]:
    ...
    if isinstance(raw_rooms, list):
        for index, raw_room in enumerate(raw_rooms):
            ...
            if (
                "floor_elevation_m" in raw_room
                and not valid_finite(raw_room.get("floor_elevation_m"))
            ):
                issues.append(
                    {
    ...
    for device in devices:
        ...
        if device["type"] in {"door", "window", "transfer"}:
    ...
    if getattr(analysis, "kind", "") in {"room_verification", "project_verification"}:
        ...
        for relationship in mapped_pressure_relationships(layout, analysis):
