...
def _air_state_from_dict(data: dict, fallback_name: str) -> AirState | UncertainAirState:
    dry_bulb = data["dry_bulb_c"]
    relative_humidity = data["relative_humidity_percent"]
    pressure = data.get("pressure_kpa", 101.325)

    if any(isinstance(value, dict) for value in (dry_bulb, relative_humidity, pressure)):
        return UncertainAirState(
            name=data.get("name", fallback_name),
            dry_bulb_c=_state_component(dry_bulb, "C"),
            relative_humidity_percent=_state_component(relative_humidity, "%"),
