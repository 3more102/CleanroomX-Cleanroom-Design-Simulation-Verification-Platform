...
def test_unsolved_fan_speed_cases_are_attention_items() -> None:
    summary = summarize_dossier_components(
        fan_speed_studies=[
            {
    ...
    component = summary["components"]["fan_speed_studies"]
    assert component["status"] == "attention_required"
    assert component["speed_case_count"] == 3
    assert component["study_count"] == 1
