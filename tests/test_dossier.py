...
def test_hvac_fan_curve_duty_states_are_preserved() -> None:
    failing_hvac = {
        "rooms": [{"air_balance": {"passes_minimum_surplus": True}}],
        "fan_curve_duty_check": {"status": "fail"},
    }
    summary = summarize_dossier_components(hvac=failing_hvac)
    assert summary["state"] == "attention_required"
    assert summary["components"]["hvac"]["fan_curve_duty_status"] == "fail"
    assert summary["adverse_items"]["hvac_fan_duty_failures"] == 1

    outside_hvac = {
        "rooms": [{"air_balance": {"passes_minimum_surplus": True}}],
        "fan_curve_duty_check": {"status": "outside_supplied_range"},
    }
    summary = summarize_dossier_components(hvac=outside_hvac)
    assert summary["state"] == "attention_required"
    assert summary["components"]["hvac"]["status"] == "outside_supplied_range"
    assert summary["adverse_items"]["hvac_fan_duty_outside_supplied_range"] == 1
...
def test_repository_demo_builds_end_to_end() -> None:
    ...
    assert result["hvac"]["fan_curve_duty_check"] is not None
    assert result["hvac"]["fan_curve_duty_check"]["status"] == "pass"
...
def test_markdown_report_includes_new_v012_sections() -> None:
    ...
    assert "Fan-curve duty check" in text
