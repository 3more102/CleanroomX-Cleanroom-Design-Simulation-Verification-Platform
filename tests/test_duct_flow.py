...
def test_pressure_balance_residual_is_zero_for_solved_paths() -> None:
    network = ParallelFlowNetwork(
        ...
        paths=(
            ...
            ParallelFlowPath("Single-section", (_section("B1", 0.45),)),
        ),
    )

    result = solve_parallel_branch_flows(network)

    assert result["max_abs_pressure_balance_error_pa"] == pytest.approx(0, abs=1e-7)
    for path in result["paths"]:
        assert path["section_pressure_drop_sum_pa"] == pytest.approx(
            path["pressure_drop_pa"], abs=1e-3
        )


def test_rectangular_parallel_sections_are_supported() -> None:
    rectangular = ParallelFlowSection(
        name="Rectangular",
        length_m=8,
        friction_factor=0.02,
        air_density_kg_m3=1.2,
    ...
    network = ParallelFlowNetwork(
        name="Mixed geometry",
        total_airflow_m3_h=2400,
        paths=(
            ParallelFlowPath("Rect", (rectangular,)),
            ParallelFlowPath("Round", (_section("Round", 0.45),)),
        ),
    )

    result = solve_parallel_branch_flows(network)

    assert result["solved_total_airflow_m3_h"] == pytest.approx(2400, abs=0.002)
    assert result["paths"][0]["airflow_m3_h"] > 0
