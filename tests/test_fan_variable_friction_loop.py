...
def test_iteration_limit_retains_terminal_bisection_evidence() -> None:
    ...
    assert trace_audit["terminal_decision_pattern_consistent"] is True
    assert trace_audit[
    ...
    assert trace_audit["terminal_replay_check"] is not None
    assert trace_audit["terminal_replay_check"][
        "terminal_bracket_matches_replayed_last_decision"
    ] is True
    ...
    assert "Terminal bracket replays the final trace decision: **True**" in report
    assert "Terminal trace outcome is consistent: **True**" in report
