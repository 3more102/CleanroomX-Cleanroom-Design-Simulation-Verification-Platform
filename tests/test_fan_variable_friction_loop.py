...
def test_independent_residual_replay_detects_terminal_residual_corruption() -> None:
    solved_study = FanVariableFrictionLoopStudy(
    ...
    solved = solve_fan_variable_friction_loop(solved_study)

    assert solved["status"] == "solved"
    evidence = solved["operating_point_search_evidence"]
    trace = evidence["bisection_trace"]
    ...
    corrupted = [dict(step) for step in trace]
    assert corrupted[-1]["decision"] == "accept_pressure_tolerance"
    tolerance = solved_study.operating_pressure_tolerance_pa
    original_terminal_residual = float(
        corrupted[-1]["midpoint_fan_minus_system_pressure_pa"]
    )
    corrupted[-1]["midpoint_fan_minus_system_pressure_pa"] = (
        -0.9 * tolerance
        if original_terminal_residual >= 0.0
        else 0.9 * tolerance
    )
    segment_index = evidence["supplied_segment_index"]
    corrupted_audit = _bisection_decision_trace_audit(
        corrupted,
        operating_iterations=solved["solver_diagnostics"][
        ...
        study=solved_study,
        segment_left=solved_study.fan_curve.points[segment_index],
        segment_right=solved_study.fan_curve.points[segment_index + 1],
    )
    ...
    limit_study = FanVariableFrictionLoopStudy(
        ...
        fan_curve=solved_study.fan_curve,
        loop_network=_fixed_network(),
        fan_discharge_node="Supply",
        fan_suction_node="Return",
        operating_pressure_tolerance_pa=1e-15,
    ...
    report = markdown_fan_variable_friction_loop_report(solved)
