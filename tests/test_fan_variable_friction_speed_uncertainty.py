...
def test_example_completes_speed_times_physical_uncertainty_matrix() -> None:
    ...
    for case in result["speed_cases"]:
        ...
        intervals = case["uncertainty_analysis"]["input_intervals"]
        assert set(intervals["edge_absolute_roughness_m"]) == {"Direct"}
        assert set(intervals["edge_kinematic_viscosity_m2_s"]) == {"Direct"}
        assert set(intervals["edge_air_density_kg_m3"]) == {"Direct"}


def test_reference_speed_case_matches_current_physical_uncertainty_engine() -> None:
...
def test_low_speed_preserves_indeterminate_uncertainty_state() -> None:
    ...
    assert low["unresolved_corner_count"] == low["corner_count"] == 8
