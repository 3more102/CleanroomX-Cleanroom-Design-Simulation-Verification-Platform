...
def test_bisection_invariant_audit_propagates_across_uncertainty_corners() -> None:
    ...
    assert summary[
        "supplied_point_network_state_hash_replay_violation_corner_indices"
    ] == []
    assert summary[
        "supplied_point_network_state_projection_replay_violation_corner_indices"
    ] == []
    assert summary[
        "supplied_point_network_state_replay_violation_corner_indices"
    ] == []
    assert summary["supplied_point_network_state_replay_violation_count"] == 0
    assert summary[
    ...
    assert system_pressure_error is not None
    assert system_pressure_error["value"] <= 2e-9
    assert system_pressure_error["sources"]
    residual_balance_error = summary[
        "maximum_bisection_trace_residual_balance_error_pa"
    ]
    assert residual_balance_error is not None
    assert residual_balance_error["value"] <= 2e-9
    assert residual_balance_error["sources"]
    ...
    assert "Supplied-point network-state replay consistent corners" in report
    assert (
    ...
    assert "Terminal network-state replay violation corners: **[]**" in report
    assert "Terminal network-state replay consistent corners" in report
    assert "Terminal network-state replay violation records across corners: **0**" in report
    assert "Exact terminal network-state replay violation details: **[]**" in report
