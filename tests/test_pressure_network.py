...
def test_solver_rejects_non_finite_intermediate_pressure_difference() -> None:
    network = RoomPressureNetwork(
        name="Overflowing pressure difference",
        nodes=(
            PressureNode("High", fixed_pressure_pa=1e308),
            PressureNode("Low", fixed_pressure_pa=-1e308),
        ),
        paths=(
            PressurePath(
                "Extreme path",
    ...
    with pytest.raises(
        RuntimeError,
        match="non-finite effective pressure difference",
    ):
